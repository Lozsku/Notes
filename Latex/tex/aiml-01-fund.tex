...
\section{Machine Learning Fundamentals}\label{machine-learning-fundamentals}
...
\subsubsection{The Classic Definition}\label{the-classic-definition}
...
\subsubsection{The Core Shift in Thinking}\label{the-core-shift-in-thinking}
...
\subsubsection{The Four Learning Paradigms}\label{the-four-learning-paradigms}
...
\paragraph{Supervised Learning --- Deep Dive}\label{supervised-learning--deep-dive}
...
\paragraph{Unsupervised Learning --- Deep Dive}\label{unsupervised-learning--deep-dive}
...
\paragraph{Self-Supervised Learning --- Deep Dive}\label{self-supervised-learning--deep-dive}
...
\paragraph{Reinforcement Learning --- Deep Dive}\label{reinforcement-learning--deep-dive}
...
\subsubsection{The Scaling Argument --- Why Rules Don\textquotesingle t Work}\label{the-scaling-argument--why-rules-dont-work}
...
\subsubsection{The Statistical Argument}\label{the-statistical-argument}
...
\subsubsection{The Economic Argument}\label{the-economic-argument}
...
\subsubsection{The End-to-End ML Lifecycle}\label{the-end-to-end-ml-lifecycle}
...
\subsubsection{Train / Validation / Test Split}\label{train--validation--test-split}
...
\subsubsection{The Bias--Variance Tradeoff}\label{the-biasvariance-tradeoff}
...
\subsubsection{Overfitting vs Underfitting}\label{overfitting-vs-underfitting}
...
\paragraph{L1 Regularization (Lasso)}\label{l1-regularization-lasso}
...
\paragraph{L2 Regularization (Ridge)}\label{l2-regularization-ridge}
...
\paragraph{L1 vs L2 Comparison}\label{l1-vs-l2-comparison}
...
\paragraph{Dropout (Neural Networks)}\label{dropout-neural-networks}
...
\subsubsection{Evaluation Metrics --- Classification}\label{evaluation-metrics--classification}
...
\paragraph{Core Metrics}\label{core-metrics}
...
\paragraph{Precision vs Recall Tradeoff}\label{precision-vs-recall-tradeoff}
...
\paragraph{ROC-AUC vs PR-AUC}\label{roc-auc-vs-pr-auc}
...
\paragraph{Log Loss (Cross-Entropy Loss)}\label{log-loss-cross-entropy-loss}
...
\subsubsection{Evaluation Metrics --- Regression}\label{evaluation-metrics--regression}
...
\subsubsection{Feature Scaling / Normalization}\label{feature-scaling--normalization}
...
\paragraph{Batch vs SGD vs Mini-Batch}\label{batch-vs-sgd-vs-mini-batch}
...
\subsubsection{The Curse of Dimensionality}\label{the-curse-of-dimensionality}
...
\subsubsection{The Full ML Workflow Pipeline}\label{the-full-ml-workflow-pipeline}
...
\subsubsection{Train / Validation / Test Split Diagram}\label{train--validation--test-split-diagram}
...
\subsubsection{Bias--Variance Curve}\label{biasvariance-curve}
...
\subsubsection{Confusion Matrix (Fraud Detection Example)}\label{confusion-matrix-fraud-detection-example}
...
\subsubsection{Gradient Descent on Loss Bowl}\label{gradient-descent-on-loss-bowl}
...
\subsubsection{Overfitting / Underfitting Fit Curves}\label{overfitting--underfitting-fit-curves}
...
\subsubsection{Google Search --- Learning to Rank}\label{google-search--learning-to-rank}
...
\subsubsection{Netflix Recommendations --- Collaborative Filtering}\label{netflix-recommendations--collaborative-filtering}
...
\subsubsection{Amazon Fraud Detection --- Anomaly + Supervised}\label{amazon-fraud-detection--anomaly--supervised}
...
\subsubsection{Uber ETA Prediction --- Regression}\label{uber-eta-prediction--regression}
...
\subsection{Real-Life Analogies --- Training a Student Through School}\label{real-life-analogies--training-a-student-through-school}
...
\subsubsection{Bias vs Variance}\label{bias-vs-variance}
...
\subsubsection{L1 vs L2 Regularization}\label{l1-vs-l2-regularization}
...
\subsubsection{Type I vs Type II Error}\label{type-i-vs-type-ii-error}
...
\subsubsection{Confusion Matrix: TN, FP, FN, TP (reading order)}\label{confusion-matrix-tn-fp-fn-tp-reading-order}
...
\subsubsection{Gradient Descent Learning Rate}\label{gradient-descent-learning-rate}
...
\subsubsection{Overfitting vs Underfitting}\label{overfitting-vs-underfitting-1}

\textbf{"Overfit = Over-smart student who studied only past papers. Underfit = Under-prepared student who barely studied."}

\subsubsection{The Bias-Variance-Noise Decomposition}\label{the-bias-variance-noise-decomposition}
...
\subsubsection{Q1: What is the bias-variance tradeoff?}\label{q1-what-is-the-bias-variance-tradeoff}
...
\subsubsection{Q2: Explain precision and recall. When would you use each?}\label{q2-explain-precision-and-recall-when-would-you-use-each}
...
\subsubsection{Q3: What is data leakage and how do you detect it?}\label{q3-what-is-data-leakage-and-how-do-you-detect-it}
...
\subsubsection{Q4: Your model has 98\% accuracy but the business is unhappy. What\textquotesingle s wrong?}\label{q4-your-model-has-98-accuracy-but-the-business-is-unhappy-whats-wrong}
...
\subsubsection{Q5: Explain the difference between L1 and L2 regularization.}\label{q5-explain-the-difference-between-l1-and-l2-regularization}
...
\subsubsection{Q6: What is cross-validation and when do you use it?}\label{q6-what-is-cross-validation-and-when-do-you-use-it}
...
\subsubsection{1. The Learning Curve as a Diagnostic Tool}\label{1-the-learning-curve-as-a-diagnostic-tool}
...
\subsubsection{2. The No Free Lunch Theorem}\label{2-the-no-free-lunch-theorem}
...
\subsubsection{3. Statistical Significance of Evaluation Results}\label{3-statistical-significance-of-evaluation-results}
...
\subsubsection{4. Calibration vs. Discrimination}\label{4-calibration-vs-discrimination}
...
\subsubsection{5. Distributional Shift --- The Biggest Production Failure Mode}\label{5-distributional-shift--the-biggest-production-failure-mode}
...
\subsubsection{6. Inductive Bias and Regularization as Two Sides of a Coin}\label{6-inductive-bias-and-regularization-as-two-sides-of-a-coin}
...
\subsubsection{7. Hyperparameter Optimization at Scale}\label{7-hyperparameter-optimization-at-scale}
...
\subsubsection{1. Using Test Set for Validation}\label{1-using-test-set-for-validation}
...
\subsubsection{2. Ignoring Class Imbalance}\label{2-ignoring-class-imbalance}
...
\subsubsection{3. Not Scaling Features Before Distance-Based Algorithms}\label{3-not-scaling-features-before-distance-based-algorithms}
...
\subsubsection{4. Fitting Preprocessors on All Data}\label{4-fitting-preprocessors-on-all-data}
...
\subsubsection{5. Confusing Correlation with Causation in Features}\label{5-confusing-correlation-with-causation-in-features}
...
\subsubsection{6. Not Checking for Temporal Leakage}\label{6-not-checking-for-temporal-leakage}
...
\subsubsection{7. Confusing Parameters vs. Hyperparameters}\label{7-confusing-parameters-vs-hyperparameters}
...
\subsubsection{\texorpdfstring{8. Misinterpreting R\(^2\)}{8. Misinterpreting R\^{}2}}\label{8-misinterpreting-r2}
...
\subsubsection{9. Reporting Only a Single Metric}\label{9-reporting-only-a-single-metric}
...
\subsubsection{10. Treating Gradient Descent Convergence as Guaranteed}\label{10-treating-gradient-descent-convergence-as-guaranteed}
...
\subsubsection{MLOps}\label{mlops}
...
\subsubsection{Common Traps}\label{common-traps}
...
\subsubsection{Key Points Checklist}\label{key-points-checklist}
