...
\section{Why FAANG Cares (Per Company)}\label{why-faang-cares-per-company}
...
\subsection{1. OSI Model vs TCP/IP Model}\label{1-osi-model-vs-tcpip-model}
...
\subsubsection{OSI Model (7 Layers)}\label{osi-model-7-layers}
...
\subsubsection{TCP/IP Model (4 Layers)}\label{tcpip-model-4-layers}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Application     }\OperatorTok{=}\NormalTok{ OSI Layers }\DecValTok{5}\OperatorTok{+}\DecValTok{6}\OperatorTok{+}\DecValTok{7}
\NormalTok{Transport       }\OperatorTok{=}\NormalTok{ OSI Layer }\DecValTok{4}
\NormalTok{Internet        }\OperatorTok{=}\NormalTok{ OSI Layer }\DecValTok{3}
\NormalTok{Network Access  }\OperatorTok{=}\NormalTok{ OSI Layers }\DecValTok{1}\OperatorTok{+}\DecValTok{2}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{Data encapsulation flow:}\label{data-encapsulation-flow}
...
\subsection{2. IP Addressing, Subnetting, CIDR, NAT}\label{2-ip-addressing-subnetting-cidr-nat}

\subsubsection{IPv4 Addressing}\label{ipv4-addressing}
...
\subsubsection{CIDR (Classless Inter-Domain Routing)}\label{cidr-classless-inter-domain-routing}
...
\subsubsection{Private IP Ranges (RFC 1918)}\label{private-ip-ranges-rfc-1918}
...
\subsubsection{NAT (Network Address Translation)}\label{nat-network-address-translation}
...
\subsection{3. TCP vs UDP}\label{3-tcp-vs-udp}

\subsubsection{TCP (Transmission Control Protocol)}\label{tcp-transmission-control-protocol}
...
\subsubsection{UDP (User Datagram Protocol)}\label{udp-user-datagram-protocol}
...
\subsubsection{TCP vs UDP Comparison Table}\label{tcp-vs-udp-comparison-table}
...
\subsection{4. DNS (Domain Name System)}\label{4-dns-domain-name-system}

DNS is the \textbf{phone book of the internet} --- maps human-readable names to IP addresses.

\subsubsection{DNS Hierarchy}\label{dns-hierarchy}
...
\subsubsection{DNS Record Types}\label{dns-record-types}
...
\subsubsection{DNS Resolution Flow (Full Recursive)}\label{dns-resolution-flow-full-recursive}

\visualfigure{../../figures/03-networks/03-dns-resolution.pdf}{The resolver does the legwork: it walks the hierarchy (root $\rightarrow$ TLD $\rightarrow$ authoritative) on the client's behalf, then caches the answer for the TTL.}
...
\subsection{5. HTTP --- HyperText Transfer Protocol}\label{5-http--hypertext-transfer-protocol}

\subsubsection{HTTP Methods}\label{http-methods}
...
\subsubsection{HTTP Status Codes}\label{http-status-codes}
...
\subsubsection{Key HTTP Headers}\label{key-http-headers}

\setcodelang{Python}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Request headers:}
\NormalTok{  Host: www.example.com}
\NormalTok{  Authorization: Bearer }\OperatorTok{\textless{}}\NormalTok{token}\OperatorTok{\textgreater{}}
\NormalTok{  Content}\OperatorTok{{-}}\NormalTok{Type: application}\OperatorTok{/}\NormalTok{json}
\NormalTok{  Accept: application}\OperatorTok{/}\NormalTok{json}
\NormalTok{  Cache}\OperatorTok{{-}}\NormalTok{Control: no}\OperatorTok{{-}}\NormalTok{cache}
\NormalTok{  If}\OperatorTok{{-}}\VariableTok{None}\OperatorTok{{-}}\NormalTok{Match: }\StringTok{"abc123"}\NormalTok{  (conditional GET)}
\NormalTok{  Cookie: session}\OperatorTok{=}\NormalTok{abc}
\NormalTok{  User}\OperatorTok{{-}}\NormalTok{Agent: Mozilla}\OperatorTok{/}\FloatTok{5.0}\NormalTok{...}
\NormalTok{  Origin: https:}\OperatorTok{//}\NormalTok{app.example.com  (CORS)}
\NormalTok{  Connection: keep}\OperatorTok{{-}}\NormalTok{alive}

\NormalTok{Response headers:}
\NormalTok{  Content}\OperatorTok{{-}}\NormalTok{Type: application}\OperatorTok{/}\NormalTok{json}\OperatorTok{;}\NormalTok{ charset}\OperatorTok{=}\NormalTok{utf}\OperatorTok{{-}}\DecValTok{8}
\NormalTok{  Cache}\OperatorTok{{-}}\NormalTok{Control: }\BuiltInTok{max}\OperatorTok{{-}}\NormalTok{age}\OperatorTok{=}\DecValTok{3600}\NormalTok{, public}
\NormalTok{  ETag: }\StringTok{"abc123"}
\NormalTok{  Set}\OperatorTok{{-}}\NormalTok{Cookie: session}\OperatorTok{=}\NormalTok{xyz}\OperatorTok{;}\NormalTok{ HttpOnly}\OperatorTok{;}\NormalTok{ Secure}\OperatorTok{;}\NormalTok{ SameSite}\OperatorTok{=}\NormalTok{Strict}
\NormalTok{  X}\OperatorTok{{-}}\NormalTok{RateLimit}\OperatorTok{{-}}\NormalTok{Remaining: }\DecValTok{99}
\NormalTok{  Access}\OperatorTok{{-}}\NormalTok{Control}\OperatorTok{{-}}\NormalTok{Allow}\OperatorTok{{-}}\NormalTok{Origin: https:}\OperatorTok{//}\NormalTok{app.example.com  (CORS)}
\NormalTok{  Strict}\OperatorTok{{-}}\NormalTok{Transport}\OperatorTok{{-}}\NormalTok{Security: }\BuiltInTok{max}\OperatorTok{{-}}\NormalTok{age}\OperatorTok{=}\DecValTok{31536000}\NormalTok{  (HSTS)}
\end{Highlighting}
\end{Shaded}

\subsection{6. HTTPS and TLS/SSL}\label{6-https-and-tlsssl}

\subsubsection{Why TLS Exists}\label{why-tls-exists}
...
\subsubsection{TLS 1.3 Handshake (Step by Step)}\label{tls-13-handshake-step-by-step}
...
\subsection{7. REST APIs}\label{7-rest-apis}
...
\subsubsection{Six REST Constraints}\label{six-rest-constraints}
...
\subsubsection{REST API Design Principles}\label{rest-api-design-principles}
...
\subsection{8. gRPC}\label{8-grpc}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Service definition (proto }\BuiltInTok{file}\NormalTok{):}
\NormalTok{  service UserService \{}
\NormalTok{    rpc GetUser(GetUserRequest) returns (User)}\OperatorTok{;}
\NormalTok{    rpc StreamUsers(Empty) returns (stream User)}\OperatorTok{;}
\NormalTok{  \}}

\NormalTok{Proto compiler generates:}
  \OperatorTok{{-}}\NormalTok{ Client stub (}\KeywordTok{in} \BuiltInTok{any}\NormalTok{ language)}
  \OperatorTok{{-}}\NormalTok{ Server interface to implement}
  \OperatorTok{{-}}\NormalTok{ Serialization}\OperatorTok{/}\NormalTok{deserialization code}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{REST vs gRPC vs GraphQL}\label{rest-vs-grpc-vs-graphql}
...
\subsection{9. WebSockets, SSE, and Long-Polling}\label{9-websockets-sse-and-long-polling}
...
\subsubsection{Long-Polling}\label{long-polling}
...
\subsubsection{Server-Sent Events (SSE)}\label{server-sent-events-sse}

\setcodelang{Python}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Client: GET }\OperatorTok{/}\NormalTok{events}
\NormalTok{Server: Content}\OperatorTok{{-}}\NormalTok{Type: text}\OperatorTok{/}\NormalTok{event}\OperatorTok{{-}}\NormalTok{stream}

\NormalTok{data: \{}\StringTok{"type"}\NormalTok{:}\StringTok{"price\_update"}\NormalTok{,}\StringTok{"value"}\NormalTok{:}\FloatTok{150.23}\NormalTok{\}\textbackslash{}n\textbackslash{}n}
\NormalTok{data: \{}\StringTok{"type"}\NormalTok{:}\StringTok{"price\_update"}\NormalTok{,}\StringTok{"value"}\NormalTok{:}\FloatTok{150.50}\NormalTok{\}\textbackslash{}n\textbackslash{}n}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{WebSockets}\label{websockets}
...
\subsubsection{Comparison: Polling vs SSE vs WebSocket}\label{comparison-polling-vs-sse-vs-websocket}
...
\subsection{10. HTTP/1.1 vs HTTP/2 vs HTTP/3}\label{10-http11-vs-http2-vs-http3}

\subsubsection{HTTP/1.1 Problems}\label{http11-problems}
...
\subsubsection{HTTP/2 Solutions}\label{http2-solutions}
...
\subsubsection{HTTP/3 (QUIC)}\label{http3-quic}
...
\subsubsection{HTTP Comparison Table}\label{http-comparison-table}
...
\subsection{11. Load Balancing}\label{11-load-balancing}

\subsubsection{Why Load Balancing}\label{why-load-balancing}
...
\subsubsection{L4 vs L7 Load Balancing}\label{l4-vs-l7-load-balancing}
...
\subsubsection{Load Balancing Algorithms}\label{load-balancing-algorithms}
...
\subsubsection{Health Checks}\label{health-checks}
...
\subsubsection{Topology Options}\label{topology-options}
...
\subsection{12. CORS (Cross-Origin Resource Sharing)}\label{12-cors-cross-origin-resource-sharing}
...
\subsubsection{Simple vs Preflight Requests}\label{simple-vs-preflight-requests}
...
\subsection{OSI Stack Diagram}\label{osi-stack-diagram}
...
\subsection{TCP 3-Way Handshake}\label{tcp-3-way-handshake}

\visualfigure{../../figures/03-networks/02-tcp-handshake.pdf}{TCP opens with SYN / SYN-ACK / ACK to agree on sequence numbers, then tears down with a four-way FIN/ACK exchange. This reliability is what UDP trades away for speed.}
...
\subsection{TLS 1.3 Handshake}\label{tls-13-handshake}
...
\subsection{DNS Resolution Flow}\label{dns-resolution-flow}
...
\subsection{HTTP/2 Multiplexing}\label{http2-multiplexing}
...
\subsection{Load Balancer Topology}\label{load-balancer-topology}
...
\subsection{OSI Layers (Top to Bottom)}\label{osi-layers-top-to-bottom}
...
\subsection{TCP Flags}\label{tcp-flags}
...
\subsection{HTTP Status Code Families}\label{http-status-code-families}
...
\subsection{TCP Handshake}\label{tcp-handshake}

\textbf{SYN \(\rightarrow\) SYN-ACK \(\rightarrow\) ACK} = \textbf{"See ya!" \(\rightarrow\) "See ya, and you!" \(\rightarrow\) "You!"}

\subsection{\texorpdfstring{REST HTTP Methods \(\rightarrow\) CRUD}{REST HTTP Methods \textbackslash rightarrow CRUD}}\label{rest-http-methods-rightarrow-crud}
...
\subsection{CIDR quick math}\label{cidr-quick-math}
...
\subsection{\texorpdfstring{Q1: "What happens when you type \url{https://www.google.com} in your browser and press Enter?"}{Q1: "What happens when you type https://www.google.com in your browser and press Enter?"}}\label{q1-what-happens-when-you-type-httpswwwgooglecom-in-your-browser-and-press-enter}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{GET }\OperatorTok{/}\NormalTok{ HTTP}\OperatorTok{/}\DecValTok{2}
\NormalTok{Host: www.google.com}
\NormalTok{Accept: text}\OperatorTok{/}\NormalTok{html,application}\OperatorTok{/}\NormalTok{xhtml}\OperatorTok{+}\NormalTok{xml}
\NormalTok{Accept}\OperatorTok{{-}}\NormalTok{Encoding: gzip, deflate, br}
\NormalTok{Accept}\OperatorTok{{-}}\NormalTok{Language: en}\OperatorTok{{-}}\NormalTok{US,en}\OperatorTok{;}\NormalTok{q}\OperatorTok{=}\FloatTok{0.9}
\NormalTok{Cookie: [existing cookies]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{HTTP}\OperatorTok{/}\DecValTok{2} \DecValTok{200}\NormalTok{ OK}
\NormalTok{Content}\OperatorTok{{-}}\NormalTok{Type: text}\OperatorTok{/}\NormalTok{html}\OperatorTok{;}\NormalTok{ charset}\OperatorTok{=}\NormalTok{UTF}\OperatorTok{{-}}\DecValTok{8}
\NormalTok{Content}\OperatorTok{{-}}\NormalTok{Encoding: br (Brotli compressed)}
\NormalTok{Cache}\OperatorTok{{-}}\NormalTok{Control: private, }\BuiltInTok{max}\OperatorTok{{-}}\NormalTok{age}\OperatorTok{=}\DecValTok{0}
\NormalTok{[HTML body]}
\end{Highlighting}
\end{Shaded}
...
\subsection{Q2: "Explain TCP flow control vs congestion control"}\label{q2-explain-tcp-flow-control-vs-congestion-control}
...
\subsection{Q3: "Why does TLS use both asymmetric and symmetric cryptography?"}\label{q3-why-does-tls-use-both-asymmetric-and-symmetric-cryptography}
...
\subsection{Q4: "REST vs gRPC --- when do you choose each?"}\label{q4-rest-vs-grpc--when-do-you-choose-each}
...
\subsection{Q5: "What is HTTP/2 head-of-line blocking and how does HTTP/3 fix it?"}\label{q5-what-is-http2-head-of-line-blocking-and-how-does-http3-fix-it}
...
\subsection{Q6: "Explain the difference between L4 and L7 load balancing. When do you use each?"}\label{q6-explain-the-difference-between-l4-and-l7-load-balancing-when-do-you-use-each}
...
\subsection{Q7: "What is CORS and why does it exist? How is it different from authentication?"}\label{q7-what-is-cors-and-why-does-it-exist-how-is-it-different-from-authentication}
...
\subsection{Key Numbers to Memorize}\label{key-numbers-to-memorize}
...
\subsection{Master Comparison Table}\label{master-comparison-table}
...
\subsection{Most Important Concepts (FAANG Top-10)}\label{most-important-concepts-faang-top-10}
