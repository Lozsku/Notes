...
\subsection{Relational Model}\label{relational-model}
...
\subsection{CAP Theorem Applied to Databases}\label{cap-theorem-applied-to-databases}
...
\subsection{Normalization vs Denormalization}\label{normalization-vs-denormalization}
...
\section{SQL Deep Dive}\label{sql-deep-dive}

\subsection{Joins --- Complete Reference}\label{joins--complete-reference}
...
\subsubsection{INNER JOIN --- Only matching rows}\label{inner-join--only-matching-rows}
...
\subsubsection{LEFT JOIN --- All left rows + matching right (NULLs if no match)}\label{left-join--all-left-rows--matching-right-nulls-if-no-match}
...
\subsubsection{RIGHT JOIN --- All right rows + matching left}\label{right-join--all-right-rows--matching-left}
...
\subsubsection{FULL OUTER JOIN --- Union of LEFT + RIGHT}\label{full-outer-join--union-of-left--right}
...
\subsubsection{\texorpdfstring{CROSS JOIN --- Cartesian product (M \(\times\) N rows)}{CROSS JOIN --- Cartesian product (M \textbackslash times N rows)}}\label{cross-join--cartesian-product-m-times-n-rows}
...
\subsubsection{SELF JOIN --- Table joined with itself}\label{self-join--table-joined-with-itself}
...
\subsubsection{Anti-Join pattern (find rows with no match)}\label{anti-join-pattern-find-rows-with-no-match}
...
\subsection{CTEs (Common Table Expressions)}\label{ctes-common-table-expressions}
...
\subsubsection{Non-Recursive CTE Example}\label{non-recursive-cte-example}
...
\subsubsection{Recursive CTE Example --- Org chart traversal}\label{recursive-cte-example--org-chart-traversal}
...
\subsection{Query Execution Plan / EXPLAIN}\label{query-execution-plan--explain}
...
\section{Indexing \& Internals}\label{indexing--internals}

\subsection{B-Tree vs B+Tree vs LSM-Tree}\label{b-tree-vs-btree-vs-lsm-tree}

\subsubsection{B+Tree (Used in PostgreSQL, MySQL InnoDB, SQL Server)}\label{btree-used-in-postgresql-mysql-innodb-sql-server}
...
\subsubsection{LSM-Tree (Log-Structured Merge-Tree) --- Used in Cassandra, RocksDB, LevelDB, HBase}\label{lsm-tree-log-structured-merge-tree--used-in-cassandra-rocksdb-leveldb-hbase}
...
\subsection{Clustered vs Non-Clustered Index}\label{clustered-vs-non-clustered-index}

\subsubsection{Clustered Index}\label{clustered-index}
...
\subsubsection{Non-Clustered (Secondary) Index}\label{non-clustered-secondary-index}
...
\subsection{How Index Speeds Lookups}\label{how-index-speeds-lookups}
...
\section{ACID, Isolation Levels \& Locking}\label{acid-isolation-levels--locking}

\subsection{ACID Properties}\label{acid-properties}
...
\subsection{\texorpdfstring{THE Isolation Level \(\times\) Anomaly Matrix}{THE Isolation Level \textbackslash times Anomaly Matrix}}\label{the-isolation-level-times-anomaly-matrix}
...
\subsubsection{Anomaly Definitions}\label{anomaly-definitions}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{T1: read balance}\OperatorTok{=}\DecValTok{1000}\NormalTok{, compute }\DecValTok{1000}\OperatorTok{+}\DecValTok{100}\OperatorTok{=}\DecValTok{1100}
\NormalTok{T2: read balance}\OperatorTok{=}\DecValTok{1000}\NormalTok{, compute }\DecValTok{1000}\OperatorTok{+}\DecValTok{200}\OperatorTok{=}\DecValTok{1200}\NormalTok{, write }\DecValTok{1200}\OperatorTok{;}\NormalTok{ COMMIT}
\NormalTok{T1: write }\DecValTok{1100}\OperatorTok{;}\NormalTok{ COMMIT}
\NormalTok{Balance }\OperatorTok{=} \DecValTok{1100}\NormalTok{ — T2}\StringTok{\textquotesingle{}s +200 is LOST!}
\end{Highlighting}
\end{Shaded}

\subsection{MVCC (Multi-Version Concurrency Control)}\label{mvcc-multi-version-concurrency-control}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{PostgreSQL MVCC (simplified):}
\NormalTok{  Row has: xmin (created by tx), xmax (deleted}\OperatorTok{/}\NormalTok{updated by tx)}

\NormalTok{  INSERT (tx }\DecValTok{100}\NormalTok{): row \{xmin}\OperatorTok{=}\DecValTok{100}\NormalTok{, xmax}\OperatorTok{=}\NormalTok{NULL, data}\OperatorTok{=}\StringTok{"Alice, 90000"}\NormalTok{\}}
\NormalTok{  UPDATE (tx }\DecValTok{200}\NormalTok{): old row \{xmin}\OperatorTok{=}\DecValTok{100}\NormalTok{, xmax}\OperatorTok{=}\DecValTok{200}\NormalTok{, data}\OperatorTok{=}\StringTok{"Alice, 90000"}\NormalTok{\}}
\NormalTok{                   new row \{xmin}\OperatorTok{=}\DecValTok{200}\NormalTok{, xmax}\OperatorTok{=}\NormalTok{NULL, data}\OperatorTok{=}\StringTok{"Alice, 100000"}\NormalTok{\}}

\NormalTok{  Tx }\DecValTok{150}\NormalTok{ (started before tx }\DecValTok{200}\NormalTok{): sees old row (xmin}\OperatorTok{=}\DecValTok{100} \OperatorTok{\textless{}} \DecValTok{150}\NormalTok{, xmax}\OperatorTok{=}\DecValTok{200} \OperatorTok{\textgreater{}} \DecValTok{150}\NormalTok{)}
\NormalTok{  Tx }\DecValTok{250}\NormalTok{ (started after tx }\DecValTok{200}\NormalTok{ commits): sees new row (xmin}\OperatorTok{=}\DecValTok{200} \OperatorTok{\textless{}} \DecValTok{250}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\subsection{Optimistic vs Pessimistic Locking}\label{optimistic-vs-pessimistic-locking}
...
\subsection{Deadlocks in Databases}\label{deadlocks-in-databases}
...
\subsection{Lock Types}\label{lock-types}
...
\subsection{B+Tree Structure (Full ASCII)}\label{btree-structure-full-ascii}
...
\subsection{LSM-Tree Compaction Flow}\label{lsm-tree-compaction-flow}
...
\subsection{Replication Topology}\label{replication-topology}
...
\subsection{Sharding / Partitioning}\label{sharding--partitioning}
...
\section{NoSQL Deep Dive}\label{nosql-deep-dive}

\subsection{When to Pick Which Database}\label{when-to-pick-which-database}
...
\subsection{NoSQL Families Comparison}\label{nosql-families-comparison}
...
\subsection{Cassandra Architecture}\label{cassandra-architecture}
...
\subsection{DynamoDB}\label{dynamodb}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{PK               }\OperatorTok{|}\NormalTok{ SK               }\OperatorTok{|}\NormalTok{ data}
\NormalTok{USER}\CommentTok{\#alice       | PROFILE          | \{name, email\}}
\NormalTok{USER}\CommentTok{\#alice       | ORDER\#2024{-}01{-}01 | \{items, total\}}
\NormalTok{PRODUCT}\CommentTok{\#widget   | META             | \{price, desc\}}
\end{Highlighting}
\end{Shaded}
...
\subsection{Isolation Levels (weakest to strongest)}\label{isolation-levels-weakest-to-strongest}
...
\subsection{Joins Memory Aid}\label{joins-memory-aid}
...
\subsection{B+Tree vs LSM Choice}\label{btree-vs-lsm-choice}
...
\subsection{Window Function Rank Types}\label{window-function-rank-types}
...
\subsection{NoSQL Family by Access Pattern}\label{nosql-family-by-access-pattern}
...
\subsection{SQL Query Questions (Frequently Asked)}\label{sql-query-questions-frequently-asked}
...
\subsection{System-Level DB Questions}\label{system-level-db-questions}
...
\subsection{Follow-Up Questions Interviewers Love}\label{follow-up-questions-interviewers-love}
...
\subsection{Index Trade-offs}\label{index-trade-offs}
...
\subsection{Write vs Read Optimization}\label{write-vs-read-optimization}
...
\subsection{Hot Partitions}\label{hot-partitions}
...
\subsection{Replication Lag}\label{replication-lag}
...
\subsection{Query Optimization Workflow}\label{query-optimization-workflow}
...
\visualfigure{../figures/08-databases/01-btree.pdf}{B+ trees keep data sorted with a high branching factor, so lookups touch only O(log n) nodes (few disk pages). Linked leaves make range queries cheap.}
\visualfigure{../figures/08-databases/02-isolation-levels.pdf}{Stronger isolation prevents more anomalies but costs concurrency. Serializable is safest; most OLTP systems default to Read Committed and add locks where needed.}
\end{document}
