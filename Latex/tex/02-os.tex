...
\section{Operating Systems --- FAANG Interview Mastery}\label{operating-systems--faang-interview-mastery}
...
\subsection{Why FAANG Cares (Specifics)}\label{why-faang-cares-specifics}
...
\subsubsection{Kernel vs User Mode}\label{kernel-vs-user-mode}
...
\subsubsection{Process vs Thread}\label{process-vs-thread}
...
\paragraph{Key Differences Table}\label{key-differences-table}
...
\subsubsection{Process States and Lifecycle}\label{process-states-and-lifecycle}
...
\subsubsection{fork() and exec()}\label{fork-and-exec}
...
\subsubsection{Context Switching}\label{context-switching}
...
\subsubsection{Scheduling Algorithms}\label{scheduling-algorithms}
...
\paragraph{FCFS --- First Come First Served}\label{fcfs--first-come-first-served}
...
\paragraph{SJF --- Shortest Job First}\label{sjf--shortest-job-first}
...
\paragraph{Round Robin (RR)}\label{round-robin-rr}
...
\paragraph{Priority Scheduling}\label{priority-scheduling}
...
\paragraph{MLFQ --- Multi-Level Feedback Queue}\label{mlfq--multi-level-feedback-queue}
...
\paragraph{CFS --- Completely Fair Scheduler (Linux)}\label{cfs--completely-fair-scheduler-linux}
...
\paragraph{Scheduling Algorithm Comparison Table}\label{scheduling-algorithm-comparison-table}
...
\subsubsection{Paging}\label{paging}
...
\subsubsection{TLB --- Translation Lookaside Buffer}\label{tlb--translation-lookaside-buffer}
...
\subsubsection{Page Faults}\label{page-faults}
...
\subsubsection{Page Replacement Algorithms}\label{page-replacement-algorithms}

When RAM is full and a new page must be loaded, the OS must \textbf{evict} an existing page. Which one?

\paragraph{FIFO --- First In First Out}\label{fifo--first-in-first-out}
...
\paragraph{Optimal (OPT)}\label{optimal-opt}
...
\paragraph{LRU --- Least Recently Used}\label{lru--least-recently-used}
...
\paragraph{Clock Algorithm (Second Chance)}\label{clock-algorithm-second-chance}
...
\paragraph{Page Replacement Comparison Table}\label{page-replacement-comparison-table}
...
\subsubsection{Thrashing}\label{thrashing}
...
\subsubsection{IPC --- Inter-Process Communication}\label{ipc--inter-process-communication}

Processes are isolated. To communicate, they need OS-provided channels.

\paragraph{Pipes}\label{pipes}
...
\paragraph{Shared Memory}\label{shared-memory}
...
\paragraph{Message Queues}\label{message-queues}
...
\paragraph{Sockets}\label{sockets}
...
\paragraph{Signals}\label{signals}
...
\paragraph{IPC Comparison Table}\label{ipc-comparison-table}
...
\paragraph{Block Addressing in ext2/ext3/ext4}\label{block-addressing-in-ext2ext3ext4}
...
\paragraph{Journaling}\label{journaling}
...
\subsubsection{System Calls Deep Dive}\label{system-calls-deep-dive}
...
\subsubsection{\texorpdfstring{Complete Virtual \(\rightarrow\) Physical Address Translation}{Complete Virtual \textbackslash rightarrow Physical Address Translation}}\label{complete-virtual-rightarrow-physical-address-translation}
...
\subsubsection{Process State Machine}\label{process-state-machine}
...
\subsubsection{Memory Layout: Kernel vs User Space}\label{memory-layout-kernel-vs-user-space}
...
\subsubsection{Scheduling Gantt Charts Comparison}\label{scheduling-gantt-charts-comparison}
...
\subsubsection{Page Table Walk (4-level x86-64)}\label{page-table-walk-4-level-x86-64}
...
\subsubsection{Q1: What is the difference between a process and a thread?}\label{q1-what-is-the-difference-between-a-process-and-a-thread}
...
\subsubsection{Q2: Explain virtual memory and why it\textquotesingle s useful.}\label{q2-explain-virtual-memory-and-why-its-useful}
...
\subsubsection{Q3: How does the OS choose which process to run next?}\label{q3-how-does-the-os-choose-which-process-to-run-next}
...
\subsubsection{Q4: Explain deadlocks --- how do they occur and how do you prevent them?}\label{q4-explain-deadlocks--how-do-they-occur-and-how-do-you-prevent-them}
...
\subsubsection{Q5: What is a context switch and what makes it expensive?}\label{q5-what-is-a-context-switch-and-what-makes-it-expensive}
...
\subsubsection{Q6: Explain how fork() and copy-on-write work.}\label{q6-explain-how-fork-and-copy-on-write-work}
...
\subsubsection{Concurrency and Synchronization}\label{concurrency-and-synchronization}
...
\subsubsection{Key Numbers}\label{key-numbers}
