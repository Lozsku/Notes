...
\section{Deep Learning \& Neural Networks}\label{deep-learning--neural-networks}
...
\subsection{Why FAANG Cares (Be Specific)}\label{why-faang-cares-be-specific}
...
\subsubsection{The Perceptron --- Atomic Unit}\label{the-perceptron--atomic-unit}
...
\subsubsection{Multilayer Perceptron (MLP)}\label{multilayer-perceptron-mlp}
...
\paragraph{ReLU (Rectified Linear Unit)}\label{relu-rectified-linear-unit}
...
\paragraph{GELU (Gaussian Error Linear Unit)}\label{gelu-gaussian-error-linear-unit}
...
\paragraph{Activation Function Comparison Table}\label{activation-function-comparison-table}
...
\paragraph{Mean Squared Error (MSE) --- Regression}\label{mean-squared-error-mse--regression}
...
\paragraph{Binary Cross-Entropy --- Binary Classification}\label{binary-cross-entropy--binary-classification}
...
\paragraph{Categorical Cross-Entropy --- Multiclass}\label{categorical-cross-entropy--multiclass}
...
\paragraph{Cross-Entropy vs MSE for Classification}\label{cross-entropy-vs-mse-for-classification}
...
\subsubsection{Backpropagation \& the Chain Rule}\label{backpropagation--the-chain-rule}
...
\paragraph{SGD (Stochastic Gradient Descent)}\label{sgd-stochastic-gradient-descent}
...
\paragraph{SGD with Momentum}\label{sgd-with-momentum}
...
\paragraph{RMSprop}\label{rmsprop}
...
\paragraph{Adam (Adaptive Moment Estimation)}\label{adam-adaptive-moment-estimation}
...
\paragraph{Learning Rate Schedules}\label{learning-rate-schedules}
...
\paragraph{Optimizer Comparison}\label{optimizer-comparison}
...
\subsubsection{Vanishing \& Exploding Gradients}\label{vanishing--exploding-gradients}
...
\paragraph{Xavier/Glorot Initialization (for Tanh/Sigmoid)}\label{xavierglorot-initialization-for-tanhsigmoid}
...
\paragraph{He Initialization (for ReLU)}\label{he-initialization-for-relu}
...
\paragraph{Why initialization matters}\label{why-initialization-matters}
...
\subsubsection{Batch Normalization \& Layer Normalization}\label{batch-normalization--layer-normalization}

\paragraph{Batch Normalization (BatchNorm)}\label{batch-normalization-batchnorm}
...
\paragraph{Layer Normalization (LayerNorm)}\label{layer-normalization-layernorm}
...
\paragraph{BatchNorm vs LayerNorm}\label{batchnorm-vs-layernorm}
...
\subsubsection{Dropout \& Regularization in Deep Learning}\label{dropout--regularization-in-deep-learning}
...
\paragraph{Other DL Regularization Techniques}\label{other-dl-regularization-techniques}
...
\subsubsection{Convolutional Neural Networks (CNNs)}\label{convolutional-neural-networks-cnns}
...
\paragraph{Classic CNN Architectures}\label{classic-cnn-architectures}
...
\paragraph{Parameter Count for Convolution Layer}\label{parameter-count-for-convolution-layer}
...
\subsubsection{Recurrent Neural Networks (RNNs)}\label{recurrent-neural-networks-rnns}
...
\paragraph{LSTM (Long Short-Term Memory)}\label{lstm-long-short-term-memory}
...
\paragraph{GRU (Gated Recurrent Unit)}\label{gru-gated-recurrent-unit}
...
\paragraph{RNN vs LSTM vs GRU}\label{rnn-vs-lstm-vs-gru}
...
\subsubsection{Embeddings \& Word2Vec}\label{embeddings--word2vec}
...
\subsubsection{Transfer Learning \& Fine-Tuning}\label{transfer-learning--fine-tuning}
...
\subsubsection{Generative Adversarial Networks (GANs)}\label{generative-adversarial-networks-gans}
...
\subsubsection{Overfitting in Deep Learning}\label{overfitting-in-deep-learning}
...
\subsubsection{Single Neuron / Perceptron}\label{single-neuron--perceptron}
...
\subsubsection{MLP Architecture}\label{mlp-architecture}
...
\subsubsection{Forward + Backpropagation Flow}\label{forward--backpropagation-flow}
...
\subsubsection{Activation Function Shapes}\label{activation-function-shapes}
...
\subsubsection{Gradient Descent Visualization}\label{gradient-descent-visualization}
...
\subsubsection{RNN / LSTM Unrolled}\label{rnn--lstm-unrolled}
...
\subsection{Real-Life Analogies --- The Learning Factory Assembly Line}\label{real-life-analogies--the-learning-factory-assembly-line}
...
\subsubsection{Q1: Explain backpropagation from first principles.}\label{q1-explain-backpropagation-from-first-principles}
...
\subsubsection{Q2: Why does ReLU work better than sigmoid for hidden layers?}\label{q2-why-does-relu-work-better-than-sigmoid-for-hidden-layers}
...
\subsubsection{Q3: Explain the vanishing gradient problem and how ResNets solve it.}\label{q3-explain-the-vanishing-gradient-problem-and-how-resnets-solve-it}
...
\subsubsection{Q4: Compare Adam vs SGD. When would you use each?}\label{q4-compare-adam-vs-sgd-when-would-you-use-each}
...
\subsubsection{Q5: Why does dropout work as regularization?}\label{q5-why-does-dropout-work-as-regularization}
...
\subsubsection{Q6: What\textquotesingle s the difference between batch normalization and layer normalization?}\label{q6-whats-the-difference-between-batch-normalization-and-layer-normalization}
...
\subsubsection{Q7: How do LSTMs solve the vanishing gradient problem in RNNs?}\label{q7-how-do-lstms-solve-the-vanishing-gradient-problem-in-rnns}
...
\subsubsection{Q8: Explain transfer learning and when fine-tuning is appropriate.}\label{q8-explain-transfer-learning-and-when-fine-tuning-is-appropriate}
...
\subsubsection{1. The Double Descent Phenomenon}\label{1-the-double-descent-phenomenon}
...
\subsubsection{2. Neural Scaling Laws}\label{2-neural-scaling-laws}
...
\subsubsection{3. Loss Landscape Geometry}\label{3-loss-landscape-geometry}
...
\subsubsection{4. Gradient Checkpointing}\label{4-gradient-checkpointing}
...
\subsubsection{5. Mixed Precision Training}\label{5-mixed-precision-training}
...
\subsubsection{6. BatchNorm at Inference vs Training}\label{6-batchnorm-at-inference-vs-training}
...
\subsubsection{7. The Problem with Adam\textquotesingle s Weight Decay}\label{7-the-problem-with-adams-weight-decay}
...
\subsubsection{8. Dying ReLU in Practice}\label{8-dying-relu-in-practice}
...
\subsubsection{\texorpdfstring{\(\rightarrow\) ML Fundamentals}{\textbackslash rightarrow ML Fundamentals}}\label{rightarrow-ml-fundamentals}
...
\subsubsection{\texorpdfstring{\(\rightarrow\) Transformers / LLMs}{\textbackslash rightarrow Transformers / LLMs}}\label{rightarrow-transformers--llms}
...
\subsubsection{\texorpdfstring{\(\rightarrow\) MLOps}{\textbackslash rightarrow MLOps}}\label{rightarrow-mlops}
...
\subsubsection{\texorpdfstring{\(\rightarrow\) Performance Engineering}{\textbackslash rightarrow Performance Engineering}}\label{rightarrow-performance-engineering}
...
\subsubsection{Key Numbers to Remember}\label{key-numbers-to-remember}
...
\subsubsection{Cheat-Sheet Table: Picking the Right Tool}\label{cheat-sheet-table-picking-the-right-tool}
...
\subsubsection{Most Important Concepts (Interview Priority)}\label{most-important-concepts-interview-priority}
