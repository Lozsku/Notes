...
\section{DSA Coding Patterns \& Reusable Templates}\label{dsa-coding-patterns--reusable-templates}
...
\subsubsection{How to Use This File}\label{how-to-use-this-file}

\begin{itemize}
\tightlist
\item
  \textbf{Before interview}: read the Pattern Recognition Cheat Table (§2) until you can map a problem sentence to a pattern in under 10 seconds.
\item
  \textbf{During prep}: for each pattern in §3, memorize the trigger signal, copy the template, and solve the listed problems.
\item
  \textbf{Night before}: skim the Revision Cheat Sheet (§7).
\end{itemize}

\subsubsection{Universal Problem-Solving Workflow}\label{universal-problem-solving-workflow}
...
\subsection{Pattern Recognition Cheat Table}\label{pattern-recognition-cheat-table}
...
\subsection{The Patterns}\label{the-patterns}

\subsubsection{Two Pointers}\label{two-pointers}
...
\subsubsection{Sliding Window (Fixed + Variable)}\label{sliding-window-fixed--variable}
...
\subsubsection{Fast \& Slow Pointers (Cycle Detection)}\label{fast--slow-pointers-cycle-detection}
...
\subsubsection{Merge Intervals}\label{merge-intervals}
...
\subsubsection{Cyclic Sort}\label{cyclic-sort}
...
\subsubsection{In-place Linked List Reversal}\label{in-place-linked-list-reversal}
...
\subsubsection{BFS (Tree + Grid + Graph)}\label{bfs-tree--grid--graph}
...
\subsubsection{DFS (Tree + Grid + Graph)}\label{dfs-tree--grid--graph}
...
\subsubsection{Subsets / Permutations / Combinations}\label{subsets--permutations--combinations}
...
\subsubsection{Binary Search (+ On Answer / Monotonic Predicate)}\label{binary-search--on-answer--monotonic-predicate}
...
\subsubsection{Top-K / Heap}\label{top-k--heap}
...
\subsubsection{K-way Merge}\label{k-way-merge}
...
\subsubsection{Two Heaps (Median of Stream)}\label{two-heaps-median-of-stream}
...
\subsubsection{Monotonic Stack}\label{monotonic-stack}
...
\subsubsection{Prefix Sum}\label{prefix-sum}
...
\subsubsection{Union-Find (DSU)}\label{union-find-dsu}
...
\paragraph{DP --- 0/1 Knapsack}\label{dp--01-knapsack}
...
\paragraph{DP --- Unbounded Knapsack}\label{dp--unbounded-knapsack}
...
\paragraph{DP --- LCS (Longest Common Subsequence)}\label{dp--lcs-longest-common-subsequence}
...
\paragraph{DP --- LIS (Longest Increasing Subsequence)}\label{dp--lis-longest-increasing-subsequence}
...
\paragraph{DP --- Matrix / Grid DP}\label{dp--matrix--grid-dp}
...
\paragraph{DP --- Interval DP}\label{dp--interval-dp}
...
\paragraph{DP --- Subsequences / Strings}\label{dp--subsequences--strings}
...
\subsubsection{Big-O of Common Data Structure Operations}\label{big-o-of-common-data-structure-operations}
...
\subsubsection{Common Algorithm Complexities}\label{common-algorithm-complexities}
...
\subsubsection{Acceptable Complexity vs Input Size N}\label{acceptable-complexity-vs-input-size-n}
...
\subsection{Interview Communication Tips}\label{interview-communication-tips}

\subsubsection{Before You Code}\label{before-you-code}
...
\subsubsection{While Coding}\label{while-coding}
...
\subsubsection{After Coding}\label{after-coding}
...
\subsubsection{Recovery Tactics}\label{recovery-tactics}
...
\subsubsection{The 20 Must-Know Patterns}\label{the-20-must-know-patterns}
...
\subsubsection{Decision Tree (30-second pattern picker)}\label{decision-tree-30-second-pattern-picker}
...
\subsubsection{Quick Complexity Reminder}\label{quick-complexity-reminder}
