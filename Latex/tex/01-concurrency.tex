...
\section{Concurrency \& Multithreading --- FAANG Interview Master Notes}\label{concurrency--multithreading--faang-interview-master-notes}

\begin{quote}
\textbf{How to use this file:} Read top-to-bottom for deep understanding. Jump to §9 (Interview Questions) + §14 (Cheat Sheet) for last-minute revision.
\end{quote}
...
\subsubsection{The Hardware Reality}\label{the-hardware-reality}
...
\subsubsection{The Latency Reality}\label{the-latency-reality}
...
\subsubsection{Timeline of Need}\label{timeline-of-need}
...
\subsubsection{Threads vs Processes}\label{threads-vs-processes}
...
\subsubsection{Mutex (Mutual Exclusion Lock)}\label{mutex-mutual-exclusion-lock}
...
\subsubsection{Semaphore}\label{semaphore}
...
\paragraph{The 4 Coffman Conditions (ALL must hold for deadlock)}\label{the-4-coffman-conditions-all-must-hold-for-deadlock}
...
\subsubsection{Livelock \& Starvation}\label{livelock--starvation}
...
\subsubsection{Atomic Operations \& CAS}\label{atomic-operations--cas}
...
\subsubsection{Memory Visibility \& volatile}\label{memory-visibility--volatile}
...
\subsubsection{Spinlock}\label{spinlock}
...
\subsubsection{Condition Variables}\label{condition-variables}
...
\subsubsection{Producer-Consumer Problem}\label{producer-consumer-problem}
...
\subsubsection{Reader-Writer Problem}\label{reader-writer-problem}
...
\subsubsection{Thread Pools}\label{thread-pools}
...
\subsubsection{Python GIL (Global Interpreter Lock)}\label{python-gil-global-interpreter-lock}
...
\subsubsection{Async vs Threads}\label{async-vs-threads}
...
\subsubsection{Thread Lifecycle}\label{thread-lifecycle}
...
\subsubsection{Mutex vs Semaphore vs Monitor}\label{mutex-vs-semaphore-vs-monitor}
...
\subsubsection{Deadlock Detection: Wait-For Graph}\label{deadlock-detection-wait-for-graph}
...
\subsubsection{Lock Ordering to Prevent Deadlock}\label{lock-ordering-to-prevent-deadlock}
...
\subsubsection{CAS Operation Internals}\label{cas-operation-internals}
...
\subsubsection{Memory Model: Visibility Issue}\label{memory-model-visibility-issue}
...
\subsubsection{Thread Pool Flow}\label{thread-pool-flow}
...
\subsubsection{Google: Bigtable Tablet Server}\label{google-bigtable-tablet-server}
...
\subsubsection{Meta: Folly\textquotesingle s Concurrency Primitives}\label{meta-follys-concurrency-primitives}
...
\subsubsection{Amazon DynamoDB: Lock-Free Data Structures}\label{amazon-dynamodb-lock-free-data-structures}
...
\subsubsection{Uber: Atomic Counters for Surge Pricing}\label{uber-atomic-counters-for-surge-pricing}
...
\subsubsection{Linux Kernel: RCU (Read-Copy-Update)}\label{linux-kernel-rcu-read-copy-update}
...
\subsubsection{Java\textquotesingle s ConcurrentHashMap}\label{javas-concurrenthashmap}
...
\subsubsection{\texorpdfstring{Deadlock 4 Conditions: \textbf{"MHNC"} (Make Him Never Circulate)}{Deadlock 4 Conditions: "MHNC" (Make Him Never Circulate)}}\label{deadlock-4-conditions-mhnc-make-him-never-circulate}
...
\subsubsection{\texorpdfstring{Semaphore P/V: \textbf{"P = Post office, V = Vacate"}}{Semaphore P/V: "P = Post office, V = Vacate"}}\label{semaphore-pv-p--post-office-v--vacate}
...
\subsubsection{\texorpdfstring{Thread States: \textbf{"NEW RABBIT WAITS BUSILY THEN DIES"}}{Thread States: "NEW RABBIT WAITS BUSILY THEN DIES"}}\label{thread-states-new-rabbit-waits-busily-then-dies}
...
\subsubsection{\texorpdfstring{Happens-Before Rules (Java): \textbf{"SLVJ"} --- Start, Lock, Volatile, Join}{Happens-Before Rules (Java): "SLVJ" --- Start, Lock, Volatile, Join}}\label{happens-before-rules-java-slvj--start-lock-volatile-join}
...
\subsubsection{\texorpdfstring{CAS vs Lock: \textbf{"CAS for counters, Lock for compounds"}}{CAS vs Lock: "CAS for counters, Lock for compounds"}}\label{cas-vs-lock-cas-for-counters-lock-for-compounds}
...
\subsubsection{\texorpdfstring{Pool Sizing: \textbf{"I/O = Cores \(\times\) 10, CPU = Cores + 1"}}{Pool Sizing: "I/O = Cores \textbackslash times 10, CPU = Cores + 1"}}\label{pool-sizing-io--cores-times-10-cpu--cores--1}
...
\subsubsection{\texorpdfstring{False Sharing Fix: \textbf{"Pad to 64"}}{False Sharing Fix: "Pad to 64"}}\label{false-sharing-fix-pad-to-64}
...
\subsubsection{volatile vs synchronized:}\label{volatile-vs-synchronized}
...
\subsubsection{\texorpdfstring{Memory model mental model: \textbf{"Write \(\rightarrow\) Fence \(\rightarrow\) Read"}}{Memory model mental model: "Write \textbackslash rightarrow Fence \textbackslash rightarrow Read"}}\label{memory-model-mental-model-write-rightarrow-fence-rightarrow-read}
...
\subsubsection{Q1: What is a race condition? How do you prevent it?}\label{q1-what-is-a-race-condition-how-do-you-prevent-it}
...
\subsubsection{Q2: Explain the four conditions for deadlock and how to prevent each.}\label{q2-explain-the-four-conditions-for-deadlock-and-how-to-prevent-each}
...
\subsubsection{Q3: Implement producer-consumer with two semaphores.}\label{q3-implement-producer-consumer-with-two-semaphores}
...
\subsubsection{Q4: What is the difference between a mutex and a semaphore?}\label{q4-what-is-the-difference-between-a-mutex-and-a-semaphore}
...
\subsubsection{Q5: Explain Compare-And-Swap and the ABA problem.}\label{q5-explain-compare-and-swap-and-the-aba-problem}
...
\subsubsection{Q6: What is false sharing and how do you fix it?}\label{q6-what-is-false-sharing-and-how-do-you-fix-it}
...
\subsubsection{Q7: What is the Python GIL and when does it matter?}\label{q7-what-is-the-python-gil-and-when-does-it-matter}
...
\subsubsection{Q8: Explain volatile in Java. When is it sufficient?}\label{q8-explain-volatile-in-java-when-is-it-sufficient}
...
\subsubsection{Q9: How does Java\textquotesingle s ReadWriteLock work, and when would you not use it?}\label{q9-how-does-javas-readwritelock-work-and-when-would-you-not-use-it}
...
\subsubsection{Q10: Design a thread-safe singleton in Java.}\label{q10-design-a-thread-safe-singleton-in-java}
...
\subsubsection{\texorpdfstring{Q11: What is a condition variable and why use \texttt{while} instead of \texttt{if}?}{Q11: What is a condition variable and why use while instead of if?}}\label{q11-what-is-a-condition-variable-and-why-use-while-instead-of-if}
...
\subsubsection{Q12: Difference between thread pool\textquotesingle s fixed vs cached vs work-stealing executors?}\label{q12-difference-between-thread-pools-fixed-vs-cached-vs-work-stealing-executors}
...
\subsubsection{1. Amdahl\textquotesingle s Law --- Theoretical Speedup Limit}\label{1-amdahls-law--theoretical-speedup-limit}
...
\subsubsection{2. Memory Model Sophistication}\label{2-memory-model-sophistication}
...
\subsubsection{3. Lock-Free Algorithm Design Challenges}\label{3-lock-free-algorithm-design-challenges}
...
\subsubsection{4. Virtual Threads (Java 21 Project Loom)}\label{4-virtual-threads-java-21-project-loom}
...
\subsubsection{5. STM (Software Transactional Memory)}\label{5-stm-software-transactional-memory}
...
\subsubsection{6. Reactive/Actor Model}\label{6-reactiveactor-model}
...
\subsubsection{7. Lock Granularity Trade-offs}\label{7-lock-granularity-trade-offs}
...
\subsubsection{\texorpdfstring{1. Using \texttt{if} instead of \texttt{while} for condition variable waits}{1. Using if instead of while for condition variable waits}}\label{1-using-if-instead-of-while-for-condition-variable-waits}
...
\subsubsection{2. Wrong semaphore acquire order causing deadlock}\label{2-wrong-semaphore-acquire-order-causing-deadlock}
...
\subsubsection{\texorpdfstring{3. Checking for \texttt{==\ null} on volatile reference without synchronized block}{3. Checking for == null on volatile reference without synchronized block}}\label{3-checking-for--null-on-volatile-reference-without-synchronized-block}
...
\subsubsection{4. Forgetting to release locks (no RAII/try-finally)}\label{4-forgetting-to-release-locks-no-raiitry-finally}
...
\subsubsection{5. Using Thread.stop() or Thread.suspend() (deprecated)}\label{5-using-threadstop-or-threadsuspend-deprecated}
...
\subsubsection{6. Thinking volatile fixes race conditions for compound operations}\label{6-thinking-volatile-fixes-race-conditions-for-compound-operations}
...
\subsubsection{7. Assuming HashMap.get() is thread-safe because it\textquotesingle s "read-only"}\label{7-assuming-hashmapget-is-thread-safe-because-its-read-only}
...
\subsubsection{\texorpdfstring{8. Ignoring lock ordering \(\rightarrow\) deadlock in production}{8. Ignoring lock ordering \textbackslash rightarrow deadlock in production}}\label{8-ignoring-lock-ordering-rightarrow-deadlock-in-production}

\textbf{Symptom:} Rare deadlocks that only happen under load. \textbf{Fix:} Establish and document global lock acquisition order.

\subsubsection{9. Creating too many threads for CPU-bound work}\label{9-creating-too-many-threads-for-cpu-bound-work}
...
\subsubsection{10. Assuming thread-safe classes compose safely}\label{10-assuming-thread-safe-classes-compose-safely}
...
\subsubsection{Strategy by Interview Type}\label{strategy-by-interview-type}
...
\subsubsection{Language-Specific Tips}\label{language-specific-tips}
...
\subsubsection{Common FAANG Patterns They Test}\label{common-faang-patterns-they-test}
...
\subsubsection{What Separates Good from Great Answers}\label{what-separates-good-from-great-answers}
...
\subsubsection{10-Minute Revision Summary}\label{10-minute-revision-summary}
...
\subsubsection{Compact Cheat Sheet Table}\label{compact-cheat-sheet-table}
...
\subsubsection{Most Important Interview Concepts (Ranked by Frequency)}\label{most-important-interview-concepts-ranked-by-frequency}
