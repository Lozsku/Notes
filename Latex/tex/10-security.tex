...
\section{Security Fundamentals --- Interview Mastery Notes}\label{security-fundamentals--interview-mastery-notes}
...
\subsubsection{Authentication (AuthN)}\label{authentication-authn}
...
\subsubsection{Authorization (AuthZ)}\label{authorization-authz}
...
\subsubsection{Session vs Token Authentication}\label{session-vs-token-authentication}
...
\subsubsection{RBAC vs ABAC}\label{rbac-vs-abac}
...
\subsubsection{Password Hashing (bcrypt / Argon2 + Salt)}\label{password-hashing-bcrypt--argon2--salt}
...
\subsubsection{HMAC (Hash-based Message Authentication Code)}\label{hmac-hash-based-message-authentication-code}
...
\subsubsection{TLS / PKI / Certificates}\label{tls--pki--certificates}
...
\subsubsection{Secrets Management}\label{secrets-management}
...
\subsection{AuthN vs AuthZ (clear contrast + table --- commonly confused)}\label{authn-vs-authz-clear-contrast--table--commonly-confused}
...
\subsection{OAuth 2.0 \& JWT Deep Dive}\label{oauth-20--jwt-deep-dive}

\subsubsection{OAuth 2.0 --- What problem it solves}\label{oauth-20--what-problem-it-solves}
...
\subsubsection{OAuth 2.0 Flows}\label{oauth-20-flows}
...
\subsubsection{Authorization Code Flow with PKCE --- ASCII Sequence Diagram}\label{authorization-code-flow-with-pkce--ascii-sequence-diagram}
...
\subsubsection{OpenID Connect (OIDC)}\label{openid-connect-oidc}
...
\subsubsection{JWT (JSON Web Token) Anatomy}\label{jwt-json-web-token-anatomy}
...
\subsection{Encryption \& Hashing}\label{encryption--hashing}

\subsubsection{Symmetric vs Asymmetric vs Hashing Comparison Table}\label{symmetric-vs-asymmetric-vs-hashing-comparison-table}
...
\subsubsection{When to Use Each}\label{when-to-use-each}
...
\subsubsection{Authenticated Encryption (AES-GCM)}\label{authenticated-encryption-aes-gcm}
...
\subsubsection{CSRF vs XSS --- Comparison}\label{csrf-vs-xss--comparison}
...
\subsection{OWASP Top 10}\label{owasp-top-10}
...
\subsubsection{TLS Handshake (TLS 1.3)}\label{tls-handshake-tls-13}
...
\subsubsection{PKI Trust Chain}\label{pki-trust-chain}
...
\subsubsection{Defense-in-Depth Layers}\label{defense-in-depth-layers}
...
\subsubsection{Q1: "Design a secure login system"}\label{q1-design-a-secure-login-system}
...
\subsubsection{Q2: "How does HTTPS work?"}\label{q2-how-does-https-work}
...
\subsubsection{Q3: "Explain SQL injection and how to prevent it"}\label{q3-explain-sql-injection-and-how-to-prevent-it}
...
\subsubsection{Q4: "What is the difference between encryption and hashing?"}\label{q4-what-is-the-difference-between-encryption-and-hashing}
...
\subsubsection{Q5: "What is CSRF and how do you prevent it?"}\label{q5-what-is-csrf-and-how-do-you-prevent-it}
...
\subsubsection{Threat Modeling}\label{threat-modeling}
...
\subsubsection{Zero Trust Architecture}\label{zero-trust-architecture}
...
\subsubsection{Secrets Rotation}\label{secrets-rotation}
...
\subsubsection{Supply Chain Security (A08 deep dive)}\label{supply-chain-security-a08-deep-dive}
...
\subsubsection{Advanced JWT Attack Patterns}\label{advanced-jwt-attack-patterns}
...
\subsubsection{Networks / TLS}\label{networks--tls}
...
\subsubsection{Compliance Frameworks}\label{compliance-frameworks}
...
\subsubsection{Key Points Summary}\label{key-points-summary}
...
\subsubsection{Cheat Sheet Table --- All Key Comparisons}\label{cheat-sheet-table--all-key-comparisons}
...
\subsubsection{Most Important Concepts (for final-round FAANG)}\label{most-important-concepts-for-final-round-faang}
