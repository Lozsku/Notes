...
\section{LLM Applications \& GenAI Engineering}\label{llm-applications--genai-engineering}
...
\subsection{Why FAANG \& AI Companies Care (be specific)}\label{why-faang--ai-companies-care-be-specific}
...
\paragraph{Zero-Shot Prompting}\label{zero-shot-prompting}
...
\paragraph{Few-Shot Prompting}\label{few-shot-prompting}
...
\paragraph{Chain-of-Thought (CoT)}\label{chain-of-thought-cot}
...
\paragraph{ReAct Prompting}\label{react-prompting}
...
\paragraph{System Prompts}\label{system-prompts}
...
\paragraph{Structured / JSON Output}\label{structured--json-output}
...
\paragraph{Prompt Templates}\label{prompt-templates}
...
\paragraph{Why RAG over alternatives?}\label{why-rag-over-alternatives}
...
\paragraph{The RAG Pipeline --- Ingest Phase}\label{the-rag-pipeline--ingest-phase}
...
\paragraph{Chunking Strategies}\label{chunking-strategies}
...
\paragraph{Embeddings \& Semantic Search}\label{embeddings--semantic-search}
...
\paragraph{Vector Databases \& ANN Indexes}\label{vector-databases--ann-indexes}
...
\paragraph{The RAG Pipeline --- Query Phase}\label{the-rag-pipeline--query-phase}
...
\paragraph{Hybrid Search}\label{hybrid-search}
...
\paragraph{Reranking}\label{reranking}
...
\paragraph{RAG Evaluation}\label{rag-evaluation}
...
\subsubsection{AI Agents}\label{ai-agents}
...
\paragraph{The ReAct Loop}\label{the-react-loop}
...
\paragraph{Function Calling / Tool Use}\label{function-calling--tool-use}
...
\paragraph{Agent Memory}\label{agent-memory}
...
\paragraph{Multi-Agent Systems}\label{multi-agent-systems}
...
\paragraph{Planning}\label{planning}
...
\paragraph{Offline vs Online Evaluation}\label{offline-vs-online-evaluation}
...
\subsubsection{Guardrails \& Safety}\label{guardrails--safety}

\paragraph{Prompt Injection}\label{prompt-injection}
...
\paragraph{Jailbreaks}\label{jailbreaks}
...
\paragraph{PII \& Data Privacy}\label{pii--data-privacy}
...
\paragraph{Content Filtering Pipeline}\label{content-filtering-pipeline}
...
\subsubsection{The Build Decision --- Prompting vs RAG vs Fine-Tuning}\label{the-build-decision--prompting-vs-rag-vs-fine-tuning}
...
\subsubsection{Cost \& Latency Optimization}\label{cost--latency-optimization}
...
\paragraph{Streaming}\label{streaming}
...
\paragraph{Batching}\label{batching}
...
\paragraph{Model Routing}\label{model-routing}
...
\paragraph{Distillation \& Smaller Models}\label{distillation--smaller-models}
...
\paragraph{Context Window Management}\label{context-window-management}
...
\subsubsection{Multimodal Applications}\label{multimodal-applications}
...
\subsubsection{Common GenAI Product Patterns}\label{common-genai-product-patterns}
...
\subsubsection{Full RAG Pipeline}\label{full-rag-pipeline}
...
\subsubsection{ReAct Agent Loop}\label{react-agent-loop}
...
\subsubsection{Vector ANN Search (HNSW)}\label{vector-ann-search-hnsw}
...
\subsubsection{Function Calling Flow}\label{function-calling-flow}
...
\subsubsection{Prompting vs RAG vs Fine-Tuning Decision Tree}\label{prompting-vs-rag-vs-fine-tuning-decision-tree}
...
\subsubsection{LLM App System Architecture}\label{llm-app-system-architecture}
...
\subsubsection{ChatGPT Plugins / GPT Actions}\label{chatgpt-plugins--gpt-actions}
...
\subsubsection{Perplexity AI}\label{perplexity-ai}
...
\subsubsection{Enterprise Customer Support Bots}\label{enterprise-customer-support-bots}
...
\subsubsection{Cursor / AI Code Editors}\label{cursor--ai-code-editors}
...
\subsubsection{Harvey (Legal AI)}\label{harvey-legal-ai}
...
\subsection{Real-Life Analogies --- the Research Library}\label{real-life-analogies--the-research-library}
...
\subsubsection{Q1: "Design a RAG system for a company\textquotesingle s internal knowledge base."}\label{q1-design-a-rag-system-for-a-companys-internal-knowledge-base}
...
\subsubsection{Q2: "When would you choose prompting vs. fine-tuning vs. RAG?"}\label{q2-when-would-you-choose-prompting-vs-fine-tuning-vs-rag}
...
\subsubsection{Q3: "How do you evaluate an LLM application?"}\label{q3-how-do-you-evaluate-an-llm-application}
...
\subsubsection{Q4: "How do you prevent hallucination?"}\label{q4-how-do-you-prevent-hallucination}
...
\subsubsection{Q5: "How does an AI agent work? What are the failure modes?"}\label{q5-how-does-an-ai-agent-work-what-are-the-failure-modes}
...
\subsubsection{RAG Failure Modes (and fixes)}\label{rag-failure-modes-and-fixes}
...
\subsubsection{Eval at Scale}\label{eval-at-scale}
...
\subsubsection{Cost/Latency Tradeoffs}\label{costlatency-tradeoffs}
...
\subsubsection{Agent Reliability}\label{agent-reliability}
...
\subsubsection{Prompt Injection Defense (Deep)}\label{prompt-injection-defense-deep}
...
\subsubsection{Transformers / LLMs}\label{transformers--llms}
...
\subsubsection{Databases / Vector Search}\label{databases--vector-search}
...
\subsection{FAANG / GenAI Interview Tips}\label{faang--genai-interview-tips}
...
\subsubsection{Comparison Table: Prompting vs RAG vs Fine-Tuning}\label{comparison-table-prompting-vs-rag-vs-fine-tuning}
...
\subsubsection{Top 10 Most Important Concepts}\label{top-10-most-important-concepts}
...
\subsubsection{Quick Reference Cheat Sheet}\label{quick-reference-cheat-sheet}
