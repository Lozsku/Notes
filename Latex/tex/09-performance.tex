...
\section{09 --- Performance Engineering}\label{09--performance-engineering}
...
\subsubsection{Latency vs Throughput vs Bandwidth}\label{latency-vs-throughput-vs-bandwidth}
...
\subsubsection{Percentiles --- p50, p90, p99, p99.9}\label{percentiles--p50-p90-p99-p999}
...
\paragraph{CPU Profiling}\label{cpu-profiling}
...
\paragraph{Memory Profiling}\label{memory-profiling}
...
\paragraph{Flame Graphs}\label{flame-graphs}
...
\subsubsection{Bottleneck Analysis --- CPU vs Memory vs I/O vs Network Bound}\label{bottleneck-analysis--cpu-vs-memory-vs-io-vs-network-bound}
...
\subsubsection{Memory Leaks}\label{memory-leaks}
...
\paragraph{Cache Placement Levels}\label{cache-placement-levels}
...
\paragraph{Cache Consistency Problems}\label{cache-consistency-problems}
...
\paragraph{Read vs Write Strategies}\label{read-vs-write-strategies}
...
\subsubsection{GC Tuning}\label{gc-tuning}
...
\subsubsection{Latency Optimization Techniques}\label{latency-optimization-techniques}
...
\subsubsection{Throughput Optimization Techniques}\label{throughput-optimization-techniques}
...
\subsection{Key Laws \& Methods}\label{key-laws--methods}

\subsubsection{Amdahl\textquotesingle s Law}\label{amdahls-law}
...
\subsubsection{Little\textquotesingle s Law}\label{littles-law}
...
\subsubsection{USE Method (Brendan Gregg)}\label{use-method-brendan-gregg}
...
\subsubsection{RED Method (Tom Wilkie / Weaveworks)}\label{red-method-tom-wilkie--weaveworks}
...
\subsubsection{The Memory Hierarchy (Latency Numbers Every Engineer Should Know)}\label{the-memory-hierarchy-latency-numbers-every-engineer-should-know}
...
\subsubsection{Memory Hierarchy with Latency Numbers}\label{memory-hierarchy-with-latency-numbers}
...
\subsubsection{Flame Graph Sketch}\label{flame-graph-sketch}
...
\subsubsection{Latency Percentile Distribution}\label{latency-percentile-distribution}
...
\subsubsection{Where Bottlenecks Hide in a Request Path}\label{where-bottlenecks-hide-in-a-request-path}
...
\subsection{The Performance Investigation Workflow}\label{the-performance-investigation-workflow}
...
\subsubsection{Example 1: N+1 Query Problem (DB latency)}\label{example-1-n1-query-problem-db-latency}
...
\subsubsection{Example 2: Thread Pool Exhaustion (throughput collapse)}\label{example-2-thread-pool-exhaustion-throughput-collapse}
...
\subsubsection{Example 3: Cache Stampede at Black Friday}\label{example-3-cache-stampede-at-black-friday}
...
\subsubsection{Example 4: False Sharing in Multithreaded Counter}\label{example-4-false-sharing-in-multithreaded-counter}
...
\subsubsection{\texorpdfstring{Q1: "Your service got 10\(\times\) slower overnight. How do you debug it?"}{Q1: "Your service got 10\textbackslash times slower overnight. How do you debug it?"}}\label{q1-your-service-got-10times-slower-overnight-how-do-you-debug-it}
...
\subsubsection{Q2: "How would you design a caching layer for a social media feed?"}\label{q2-how-would-you-design-a-caching-layer-for-a-social-media-feed}
...
\subsubsection{Q3: "Explain Amdahl\textquotesingle s Law and when it applies."}\label{q3-explain-amdahls-law-and-when-it-applies}
...
\subsubsection{Q4: "What is tail latency and how do you reduce it?"}\label{q4-what-is-tail-latency-and-how-do-you-reduce-it}
...
\subsubsection{Q5: "How do you use profiling tools on a Java service in production?"}\label{q5-how-do-you-use-profiling-tools-on-a-java-service-in-production}
...
\subsubsection{Tail Latency and Service Meshes}\label{tail-latency-and-service-meshes}
...
\subsubsection{Capacity Planning}\label{capacity-planning}
...
\subsubsection{Trade-offs in Performance Work}\label{trade-offs-in-performance-work}
...
\subsubsection{The Performance vs Reliability Trade-off}\label{the-performance-vs-reliability-trade-off}
...
\subsubsection{Database Performance at Scale}\label{database-performance-at-scale}
