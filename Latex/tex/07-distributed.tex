...
\section{Distributed Systems --- Interview Mastery Guide}\label{distributed-systems--interview-mastery-guide}
...
\subsubsection{Apple / Netflix / Uber / LinkedIn}\label{apple--netflix--uber--linkedin}
...
\subsection{The 8 Fallacies of Distributed Computing}\label{the-8-fallacies-of-distributed-computing}
...
\subsubsection{PACELC}\label{pacelc}
...
\subsubsection{Quorum (R + W \textgreater{} N)}\label{quorum-r--w--n}
...
\subsubsection{Consensus: Raft}\label{consensus-raft}
...
\paragraph{Raft: Three Roles}\label{raft-three-roles}
...
\paragraph{Raft: Terms}\label{raft-terms}
...
\paragraph{Raft: Leader Election}\label{raft-leader-election}
...
\paragraph{Raft: Log Replication}\label{raft-log-replication}
...
\paragraph{Raft: Safety Guarantee}\label{raft-safety-guarantee}
...
\paragraph{Raft: Cluster Changes}\label{raft-cluster-changes}
...
\subsubsection{Consensus: Paxos}\label{consensus-paxos}

\textbf{Paxos} (Lamport, 1989/1998) --- the original consensus algorithm. Notoriously hard to understand and implement.

\paragraph{Single-Decree Paxos: Two Phases}\label{single-decree-paxos-two-phases}
...
\paragraph{Raft vs Paxos Comparison}\label{raft-vs-paxos-comparison}
...
\subsubsection{Leader Election}\label{leader-election}
...
\subsubsection{Vector Clocks and Lamport Timestamps}\label{vector-clocks-and-lamport-timestamps}

\textbf{The problem:} Distributed nodes have no shared clock. We need to reason about event ordering without a global clock.

\paragraph{Lamport Timestamps}\label{lamport-timestamps}
...
\paragraph{Vector Clocks}\label{vector-clocks}
...
\subsubsection{Two-Phase Commit (2PC) and Alternatives}\label{two-phase-commit-2pc-and-alternatives}

\textbf{2PC} --- classic distributed transaction protocol.

\paragraph{2PC Flow}\label{2pc-flow}
...
\paragraph{Three-Phase Commit (3PC)}\label{three-phase-commit-3pc}
...
\subsubsection{Distributed Transactions and Saga Pattern}\label{distributed-transactions-and-saga-pattern}
...
\subsubsection{Replication Strategies}\label{replication-strategies}
...
\subsubsection{Heartbeats and Failure Detection}\label{heartbeats-and-failure-detection}
...
\subsubsection{Distributed Caching}\label{distributed-caching}
...
\subsubsection{Kafka}\label{kafka}
...
\subsubsection{Idempotency and Exactly-Once vs At-Least-Once}\label{idempotency-and-exactly-once-vs-at-least-once}
...
\subsubsection{CAP Triangle}\label{cap-triangle}
...
\subsubsection{Consistent Hashing Ring}\label{consistent-hashing-ring}
...
\subsubsection{Raft: Leader Election and Log Replication}\label{raft-leader-election-and-log-replication}
...
\subsubsection{Quorum Reads and Writes (N=5, R=3, W=3)}\label{quorum-reads-and-writes-n5-r3-w3}
...
\subsubsection{Kafka Topic/Partition/Consumer-Group}\label{kafka-topicpartitionconsumer-group}
...
\subsubsection{CAP: "Pick 2, but P is not optional"}\label{cap-pick-2-but-p-is-not-optional}
...
\subsubsection{PACELC: "P picks A or C, ELse picks L or C"}\label{pacelc-p-picks-a-or-c-else-picks-l-or-c}
...
\subsubsection{Raft: "FELT" for leader election}\label{raft-felt-for-leader-election}
...
\subsubsection{Quorum: R + W \textgreater{} N}\label{quorum-r--w--n-1}
...
\subsubsection{Consistency Levels (strongest to weakest):}\label{consistency-levels-strongest-to-weakest}
...
\subsubsection{8 Fallacies: "NLB-STT-Z"}\label{8-fallacies-nlb-stt-z}
...
\subsubsection{2PC vs Saga}\label{2pc-vs-saga}
...
\subsubsection{Raft Safety in one line:}\label{raft-safety-in-one-line}
...
\subsubsection{Q1: "What is the CAP theorem? Give examples."}\label{q1-what-is-the-cap-theorem-give-examples}
...
\subsubsection{Q2: "Explain how Raft works."}\label{q2-explain-how-raft-works}
...
\subsubsection{Q3: "Design a distributed cache."}\label{q3-design-a-distributed-cache}
...
\subsubsection{Q4: "Explain consistent hashing and why it\textquotesingle s used."}\label{q4-explain-consistent-hashing-and-why-its-used}
...
\subsubsection{Q5: "What is idempotency and why does it matter?"}\label{q5-what-is-idempotency-and-why-does-it-matter}
...
\subsubsection{Q6: "What is split-brain and how do you prevent it?"}\label{q6-what-is-split-brain-and-how-do-you-prevent-it}
...
\subsubsection{Why Paxos Is Hard to Implement}\label{why-paxos-is-hard-to-implement}
...
\subsubsection{The Exactly-Once Myth}\label{the-exactly-once-myth}
...
\subsubsection{Trade-offs: Consistency vs. Latency}\label{trade-offs-consistency-vs-latency}
...
\subsubsection{Failure Scenarios to Discuss}\label{failure-scenarios-to-discuss}
...
\subsubsection{Gossip vs Consensus for Membership}\label{gossip-vs-consensus-for-membership}
...
\subsubsection{Comparison Tables Cheat Sheet}\label{comparison-tables-cheat-sheet}
...
\subsubsection{Most Important Concepts (Priority Order)}\label{most-important-concepts-priority-order}
