...
\section{System Design Interview --- Part 2: Advanced Problems}\label{system-design-interview--part-2-advanced-problems}
...
\subsection{1. Design a Video Streaming Service (YouTube / Netflix)}\label{1-design-a-video-streaming-service-youtube--netflix}
...
\paragraph{6.1 Upload Pipeline \& Transcoding}\label{61-upload-pipeline--transcoding}
...
\paragraph{6.2 CDN Distribution}\label{62-cdn-distribution}
...
\paragraph{6.3 Adaptive Bitrate Streaming --- HLS vs DASH}\label{63-adaptive-bitrate-streaming--hls-vs-dash}
...
\paragraph{6.4 Storage Tiers}\label{64-storage-tiers}
...
\paragraph{6.5 Recommendations (brief)}\label{65-recommendations-brief}
...
\subsection{2. Design a Ride-Hailing Service (Uber / Lyft)}\label{2-design-a-ride-hailing-service-uber--lyft}
...
\paragraph{6.1 Geospatial Indexing --- Geohash vs Quadtree vs S2}\label{61-geospatial-indexing--geohash-vs-quadtree-vs-s2}
...
\paragraph{6.2 Driver-Rider Matching}\label{62-driver-rider-matching}
...
\paragraph{6.3 Real-Time Location Updates}\label{63-real-time-location-updates}
...
\paragraph{6.4 Surge Pricing}\label{64-surge-pricing}
...
\paragraph{6.5 Trip Lifecycle --- State Machine}\label{65-trip-lifecycle--state-machine}
...
\paragraph{6.6 ETA Calculation}\label{66-eta-calculation}
...
\subsection{3. Design a Distributed Cache (like Redis/Memcached at Scale)}\label{3-design-a-distributed-cache-like-redismemcached-at-scale}
...
\paragraph{6.1 Consistent Hashing}\label{61-consistent-hashing}
...
\paragraph{6.2 Replication}\label{62-replication}
...
\paragraph{6.3 Eviction Policies (LRU and Beyond)}\label{63-eviction-policies-lru-and-beyond}
...
\paragraph{6.4 Cache Invalidation Strategies}\label{64-cache-invalidation-strategies}
...
\paragraph{6.5 Hot Key Problem}\label{65-hot-key-problem}
...
\paragraph{6.6 Cache Stampede (Thundering Herd)}\label{66-cache-stampede-thundering-herd}
...
\paragraph{6.7 Write Policies}\label{67-write-policies}
...
\paragraph{6.8 Node Failure Handling}\label{68-node-failure-handling}
...
\subsection{4. Design a Web Crawler (Google-Scale)}\label{4-design-a-web-crawler-google-scale}
...
\paragraph{6.1 URL Frontier --- Priority + Politeness}\label{61-url-frontier--priority--politeness}
...
\paragraph{6.2 URL Deduplication --- Bloom Filter vs Exact Set}\label{62-url-deduplication--bloom-filter-vs-exact-set}
...
\paragraph{6.3 DNS Resolution}\label{63-dns-resolution}
...
\paragraph{6.4 robots.txt Compliance}\label{64-robotstxt-compliance}
...
\paragraph{6.5 Trap Avoidance}\label{65-trap-avoidance}
...
\paragraph{6.6 Near-Duplicate Detection (SimHash)}\label{66-near-duplicate-detection-simhash}
...
\paragraph{6.7 Distributed Coordination}\label{67-distributed-coordination}
...
\paragraph{6.8 Freshness \& Recrawl Scheduling}\label{68-freshness--recrawl-scheduling}
