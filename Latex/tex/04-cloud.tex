...
\section{Cloud \& Infrastructure --- FAANG Interview Mastery}\label{cloud--infrastructure--faang-interview-mastery}
...
\subsubsection{The Pre-Cloud Problem}\label{the-pre-cloud-problem}
...
\subsubsection{The Solutions That Emerged}\label{the-solutions-that-emerged}
...
\subsubsection{Meta (Facebook)}\label{meta-facebook}
...
\subsubsection{Netflix}\label{netflix}
...
\subsubsection{Containers vs Virtual Machines}\label{containers-vs-virtual-machines}

\paragraph{Virtual Machines --- First Principles}\label{virtual-machines--first-principles}
...
\paragraph{Containers --- First Principles}\label{containers--first-principles}
...
\paragraph{Containers vs VMs --- Full Comparison}\label{containers-vs-vms--full-comparison}
...
\subsubsection{Container Internals}\label{container-internals}

\paragraph{Linux Namespaces}\label{linux-namespaces}
...
\paragraph{Control Groups (cgroups)}\label{control-groups-cgroups}
...
\paragraph{Union Filesystems (OverlayFS)}\label{union-filesystems-overlayfs}
...
\subsubsection{Docker Deep Dive}\label{docker-deep-dive}

\paragraph{Docker Architecture}\label{docker-architecture}
...
\paragraph{Dockerfile Best Practices}\label{dockerfile-best-practices}
...
\paragraph{Docker Networking}\label{docker-networking}
...
\subsubsection{Kubernetes Architecture}\label{kubernetes-architecture}

\paragraph{The Big Picture}\label{the-big-picture}
...
\paragraph{Control Plane Components}\label{control-plane-components}
...
\paragraph{Worker Node Components}\label{worker-node-components}
...
\subsubsection{Kubernetes Workload Resources}\label{kubernetes-workload-resources}
...
\paragraph{ConfigMaps and Secrets}\label{configmaps-and-secrets}
...
\paragraph{StatefulSets}\label{statefulsets}
...
\paragraph{Jobs and CronJobs}\label{jobs-and-cronjobs}
...
\subsubsection{Kubernetes Scheduling and Autoscaling}\label{kubernetes-scheduling-and-autoscaling}

\paragraph{Scheduling Deep Dive}\label{scheduling-deep-dive}
...
\paragraph{Horizontal Pod Autoscaler (HPA)}\label{horizontal-pod-autoscaler-hpa}
...
\paragraph{Vertical Pod Autoscaler (VPA)}\label{vertical-pod-autoscaler-vpa}
...
\paragraph{Cluster Autoscaler}\label{cluster-autoscaler}
...
\subsubsection{Service Mesh (Istio / Envoy)}\label{service-mesh-istio--envoy}

\paragraph{Why Service Meshes Exist}\label{why-service-meshes-exist}
...
\paragraph{Sidecar Pattern}\label{sidecar-pattern}
...
\paragraph{Envoy Proxy Internals}\label{envoy-proxy-internals}
...
\paragraph{Istio Features}\label{istio-features}
...
\subsubsection{CI/CD Pipelines}\label{cicd-pipelines}

\paragraph{What CI/CD Means}\label{what-cicd-means}
...
\paragraph{Typical CI/CD Pipeline}\label{typical-cicd-pipeline}
...
\paragraph{Deployment Strategies}\label{deployment-strategies}
...
\subsubsection{Infrastructure as Code}\label{infrastructure-as-code}

\paragraph{Why IaC}\label{why-iac}
...
\paragraph{Terraform}\label{terraform}
...
\paragraph{Ansible}\label{ansible}
...
\subsubsection{Monitoring, Logging, and Observability}\label{monitoring-logging-and-observability}

\paragraph{The Three Pillars of Observability}\label{the-three-pillars-of-observability}
...
\paragraph{Prometheus Architecture}\label{prometheus-architecture}
...
\paragraph{Distributed Tracing}\label{distributed-tracing}
...
\paragraph{Structured Logging}\label{structured-logging}
...
\subsubsection{SLI / SLO / SLA / Error Budgets}\label{sli--slo--sla--error-budgets}
...
\paragraph{Common SLIs}\label{common-slis}
...
\paragraph{Error Budget Math}\label{error-budget-math}
...
\subsubsection{Cloud Service Models}\label{cloud-service-models}
...
\subsubsection{Container vs VM --- Side by Side}\label{container-vs-vm--side-by-side}
...
\subsubsection{Docker Image Layers}\label{docker-image-layers}
...
\subsubsection{Kubernetes Cluster Architecture (Detailed)}\label{kubernetes-cluster-architecture-detailed}
...
\subsubsection{Service Mesh Sidecar Architecture}\label{service-mesh-sidecar-architecture}
...
\subsubsection{Observability Pipeline}\label{observability-pipeline}
...
\subsubsection{\texorpdfstring{Google: Borg \(\rightarrow\) Kubernetes}{Google: Borg \textbackslash rightarrow Kubernetes}}\label{google-borg-rightarrow-kubernetes}
...
\subsubsection{Netflix: Chaos Engineering}\label{netflix-chaos-engineering}
...
\subsubsection{\texorpdfstring{Amazon Prime Video: Microservices \(\rightarrow\) Monolith (2023)}{Amazon Prime Video: Microservices \textbackslash rightarrow Monolith (2023)}}\label{amazon-prime-video-microservices-rightarrow-monolith-2023}
...
\subsubsection{Shopify: Kubernetes Auto-Scaling on Black Friday}\label{shopify-kubernetes-auto-scaling-on-black-friday}
...
\subsubsection{Stripe: Canary Deployments}\label{stripe-canary-deployments}
...
\subsubsection{Kubernetes Components: "EAS-CSK"}\label{kubernetes-components-eas-csk}
...
\subsubsection{Namespace types: "PUMNIC+T"}\label{namespace-types-pumnict}

\textbf{P}ID, \textbf{U}TS, \textbf{M}ount, \textbf{N}et, \textbf{I}PC, \textbf{C}group, \textbf{T}ime (user)

\subsubsection{Three Pillars of Observability: "MLT"}\label{three-pillars-of-observability-mlt}
...
\subsubsection{Deployment Strategies: "BCR-R"}\label{deployment-strategies-bcr-r}
...
\subsubsection{\texorpdfstring{SLI/SLO/SLA: "Indicator \(\rightarrow\) Objective \(\rightarrow\) Agreement"}{SLI/SLO/SLA: "Indicator \textbackslash rightarrow Objective \textbackslash rightarrow Agreement"}}\label{slislosla-indicator-rightarrow-objective-rightarrow-agreement}
...
\subsubsection{Service Types K8s: "CLNE-H"}\label{service-types-k8s-clne-h}
...
\subsubsection{\texorpdfstring{IaC: "T = Terra(multi) \(\rightarrow\) CFN = Amazon-native"}{IaC: "T = Terra(multi) \textbackslash rightarrow CFN = Amazon-native"}}\label{iac-t--terramulti-rightarrow-cfn--amazon-native}
...
\subsubsection{Q1: What is the difference between a container and a virtual machine?}\label{q1-what-is-the-difference-between-a-container-and-a-virtual-machine}
...
\subsubsection{Q2: Walk me through how Kubernetes schedules a pod.}\label{q2-walk-me-through-how-kubernetes-schedules-a-pod}
...
\subsubsection{Q3: Explain the difference between a Deployment and a StatefulSet.}\label{q3-explain-the-difference-between-a-deployment-and-a-statefulset}
...
\subsubsection{Q4: How does a Service Mesh work? Why use it instead of client-side libraries?}\label{q4-how-does-a-service-mesh-work-why-use-it-instead-of-client-side-libraries}
...
\subsubsection{Q5: Explain canary vs blue-green deployments. When would you choose each?}\label{q5-explain-canary-vs-blue-green-deployments-when-would-you-choose-each}
...
\subsubsection{Q6: What are the three pillars of observability? How do they relate?}\label{q6-what-are-the-three-pillars-of-observability-how-do-they-relate}
...
\subsubsection{Q7: What are SLOs and error budgets? How do you use them?}\label{q7-what-are-slos-and-error-budgets-how-do-you-use-them}
...
\subsubsection{Q8: How does Terraform manage state? What problems can arise?}\label{q8-how-does-terraform-manage-state-what-problems-can-arise}
...
\subsubsection{Kubernetes at Scale: What Actually Breaks}\label{kubernetes-at-scale-what-actually-breaks}
...
\subsubsection{Service Mesh Overhead and Alternatives}\label{service-mesh-overhead-and-alternatives}
...
\subsubsection{Multi-Cluster and Multi-Region Kubernetes}\label{multi-cluster-and-multi-region-kubernetes}
...
\subsubsection{Observability: What Interviewers Miss}\label{observability-what-interviewers-miss}
...
\subsubsection{Platform Engineering vs DevOps vs SRE}\label{platform-engineering-vs-devops-vs-sre}
...
\subsubsection{Cost Optimization in K8s}\label{cost-optimization-in-k8s}
...
\subsubsection{Mistake 1: Confusing Docker with Kubernetes}\label{mistake-1-confusing-docker-with-kubernetes}
...
\subsubsection{Mistake 2: Not Knowing etcd\textquotesingle s Role}\label{mistake-2-not-knowing-etcds-role}
...
\subsubsection{Mistake 3: Conflating Secrets with Security}\label{mistake-3-conflating-secrets-with-security}
...
\subsubsection{Mistake 4: HPA + VPA on Same Metric Conflict}\label{mistake-4-hpa--vpa-on-same-metric-conflict}
...
\subsubsection{Mistake 5: High-Cardinality Prometheus Labels}\label{mistake-5-high-cardinality-prometheus-labels}
...
\subsubsection{Mistake 6: Forgetting Resource Requests in Pods}\label{mistake-6-forgetting-resource-requests-in-pods}
...
\subsubsection{Mistake 7: Misunderstanding Rolling Update Safety}\label{mistake-7-misunderstanding-rolling-update-safety}
...
\subsubsection{Mistake 8: SLO = SLA = Uptime = 99.9\%}\label{mistake-8-slo--sla--uptime--999}
...
\subsubsection{Mistake 9: IaC Doesn\textquotesingle t Mean No Drift}\label{mistake-9-iac-doesnt-mean-no-drift}
...
\subsubsection{Mistake 10: Blue-Green Always Better than Canary}\label{mistake-10-blue-green-always-better-than-canary}
...
\subsubsection{What Google Looks For (SRE/Infrastructure interviews)}\label{what-google-looks-for-sreinfrastructure-interviews}
...
\subsubsection{What Meta Looks For}\label{what-meta-looks-for}
...
\subsubsection{What Amazon (AWS) Looks For}\label{what-amazon-aws-looks-for}
...
\subsubsection{What Netflix Looks For}\label{what-netflix-looks-for}
...
\subsubsection{Universal FAANG Tips}\label{universal-faang-tips}
...
\subsubsection{Key Points --- The Must-Remembers}\label{key-points--the-must-remembers}
...
\subsubsection{Cheat Sheet Table}\label{cheat-sheet-table}
...
\subsubsection{The Most Important Concepts (Rank-Ordered for FAANG)}\label{the-most-important-concepts-rank-ordered-for-faang}
