...
\section{Core Algorithms \& Complexity Analysis}\label{core-algorithms--complexity-analysis}
...
\subsubsection{Big-O / Big-Theta / Big-Omega}\label{big-o--big-theta--big-omega}
...
\subsubsection{Amortized Analysis}\label{amortized-analysis}
...
\subsubsection{Computing Time \& Space Complexity}\label{computing-time--space-complexity}
...
\subsubsection{Recursion Tree \& Master Theorem}\label{recursion-tree--master-theorem}
...
\subsubsection{\texorpdfstring{Input Size \(\rightarrow\) Acceptable Complexity}{Input Size \textbackslash rightarrow Acceptable Complexity}}\label{input-size-rightarrow-acceptable-complexity}
...
\paragraph{Comparison-Based Sorts}\label{comparison-based-sorts}
...
\paragraph{Non-Comparison Sorts}\label{non-comparison-sorts}
...
\paragraph{Sorting Comparison Table}\label{sorting-comparison-table}
...
\paragraph{BFS (Breadth-First Search)}\label{bfs-breadth-first-search}
...
\paragraph{DFS (Depth-First Search)}\label{dfs-depth-first-search}
...
\paragraph{Topological Sort}\label{topological-sort}
...
\paragraph{Dijkstra\textquotesingle s Algorithm}\label{dijkstras-algorithm}
...
\paragraph{Bellman-Ford Algorithm}\label{bellman-ford-algorithm}
...
\paragraph{Floyd-Warshall Algorithm}\label{floyd-warshall-algorithm}
...
\paragraph{Graph Algorithm Comparison Table}\label{graph-algorithm-comparison-table}
...
\paragraph{Minimum Spanning Tree (MST)}\label{minimum-spanning-tree-mst}
...
\paragraph{Union-Find (Disjoint Set Union)}\label{union-find-disjoint-set-union}
...
\paragraph{Cycle Detection}\label{cycle-detection}
...
\paragraph{Connected Components}\label{connected-components}
...
\paragraph{Bipartite Check}\label{bipartite-check}
...
\paragraph{Memoization vs Tabulation}\label{memoization-vs-tabulation}
...
\paragraph{How to Identify DP Problems}\label{how-to-identify-dp-problems}
...
\paragraph{Classic DP Families}\label{classic-dp-families}
...
\paragraph{When Greedy Works}\label{when-greedy-works}
...
\paragraph{Classic Greedy Problems}\label{classic-greedy-problems}
...
\subsubsection{Recursion \& Backtracking}\label{recursion--backtracking}

\paragraph{Recursion Template}\label{recursion-template}
...
\paragraph{Backtracking Framework}\label{backtracking-framework}
...
\subsubsection{Bit Manipulation}\label{bit-manipulation}

Computers operate on binary; bit tricks exploit this for O(1) operations.

\paragraph{Core Operations}\label{core-operations}
...
\paragraph{Key XOR Tricks}\label{key-xor-tricks}
...
\paragraph{Bitmask DP}\label{bitmask-dp}
...
\paragraph{Bit Tricks Summary}\label{bit-tricks-summary}
...
\subsubsection{Number Theory / Math}\label{number-theory--math}

\paragraph{GCD and LCM}\label{gcd-and-lcm}
...
\paragraph{Sieve of Eratosthenes}\label{sieve-of-eratosthenes}
...
\paragraph{Modular Arithmetic}\label{modular-arithmetic}
...
\paragraph{Fast Exponentiation (Binary Exponentiation)}\label{fast-exponentiation-binary-exponentiation}
...
\paragraph{Prime Factorization}\label{prime-factorization}
...
\paragraph{Useful Math Facts}\label{useful-math-facts}
...
\subsection{Algorithm Selection Cheat Table}\label{algorithm-selection-cheat-table}
...
\subsubsection{Binary Search}\label{binary-search-1}
...
\subsubsection{Dynamic Programming}\label{dynamic-programming-1}
...
\subsubsection{Complexity Analysis Mistakes}\label{complexity-analysis-mistakes}
...
\subsubsection{Graph Algorithm Mistakes}\label{graph-algorithm-mistakes}
...
\subsubsection{Sorting Mistakes}\label{sorting-mistakes}
...
\subsubsection{Dynamic Programming Mistakes}\label{dynamic-programming-mistakes}
...
\subsubsection{Binary Search Mistakes}\label{binary-search-mistakes}
...
\subsubsection{General Interview Mistakes}\label{general-interview-mistakes}
...
\subsubsection{Must-Know Complexities}\label{must-know-complexities}
...
\subsubsection{Algorithm Triggers (What to Think When You See...)}\label{algorithm-triggers-what-to-think-when-you-see}
...
\subsubsection{Golden Rules}\label{golden-rules}
