...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{function} \FunctionTok{BuildTree}\NormalTok{(data}\OperatorTok{,}\NormalTok{ depth)}\OperatorTok{:}
    \ControlFlowTok{if}\NormalTok{ stopping\_criterion\_met}\OperatorTok{:}
        \ControlFlowTok{return} \FunctionTok{Leaf}\NormalTok{(majority\_class or }\FunctionTok{mean}\NormalTok{(y))}
    
\NormalTok{    best\_feature}\OperatorTok{,}\NormalTok{ best\_threshold }\OperatorTok{=}\NormalTok{ None}\OperatorTok{,}\NormalTok{ None}
\NormalTok{    best\_gain }\OperatorTok{=} \OperatorTok{{-}}\NormalTok{inf}
    
    \ControlFlowTok{for}\NormalTok{ each feature f}\OperatorTok{:}
        \ControlFlowTok{for}\NormalTok{ each candidate threshold t}\OperatorTok{:}
\NormalTok{            gain }\OperatorTok{=} \FunctionTok{impurity}\NormalTok{(data) }\OperatorTok{{-}} \FunctionTok{weighted\_impurity}\NormalTok{(}\FunctionTok{split}\NormalTok{(data}\OperatorTok{,}\NormalTok{ f}\OperatorTok{,}\NormalTok{ t))}
            \ControlFlowTok{if}\NormalTok{ gain }\OperatorTok{\textgreater{}}\NormalTok{ best\_gain}\OperatorTok{:}
\NormalTok{                best\_gain }\OperatorTok{=}\NormalTok{ gain}
\NormalTok{                best\_feature}\OperatorTok{,}\NormalTok{ best\_threshold }\OperatorTok{=}\NormalTok{ f}\OperatorTok{,}\NormalTok{ t}
    
\NormalTok{    left}\OperatorTok{,}\NormalTok{ right }\OperatorTok{=} \FunctionTok{split}\NormalTok{(data}\OperatorTok{,}\NormalTok{ best\_feature}\OperatorTok{,}\NormalTok{ best\_threshold)}
    \ControlFlowTok{return} \BuiltInTok{Node}\NormalTok{(best\_feature}\OperatorTok{,}\NormalTok{ best\_threshold}\OperatorTok{,}
\NormalTok{                left}\OperatorTok{=}\FunctionTok{BuildTree}\NormalTok{(left}\OperatorTok{,}\NormalTok{ depth}\OperatorTok{+}\DecValTok{1}\NormalTok{)}\OperatorTok{,}
\NormalTok{                right}\OperatorTok{=}\FunctionTok{BuildTree}\NormalTok{(right}\OperatorTok{,}\NormalTok{ depth}\OperatorTok{+}\DecValTok{1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{function} \FunctionTok{RandomForest}\NormalTok{(X}\OperatorTok{,}\NormalTok{ y}\OperatorTok{,}\NormalTok{ n\_trees}\OperatorTok{,}\NormalTok{ max\_features)}\OperatorTok{:}
\NormalTok{    trees }\OperatorTok{=}\NormalTok{ []}
    \ControlFlowTok{for}\NormalTok{ i }\KeywordTok{in} \FloatTok{1.}\OperatorTok{.}\AttributeTok{n\_trees}\OperatorTok{:}
\NormalTok{        \# Bootstrap sample}
\NormalTok{        X\_boot}\OperatorTok{,}\NormalTok{ y\_boot }\OperatorTok{=} \FunctionTok{bootstrap}\NormalTok{(X}\OperatorTok{,}\NormalTok{ y)          \# sample n rows WITH replacement}
        
\NormalTok{        \# Build tree }\ControlFlowTok{with}\NormalTok{ feature subsampling}
\NormalTok{        tree }\OperatorTok{=} \FunctionTok{BuildTree}\NormalTok{(X\_boot}\OperatorTok{,}\NormalTok{ y\_boot}\OperatorTok{,}
\NormalTok{                         max\_features}\OperatorTok{=}\FunctionTok{sqrt}\NormalTok{(p))     \# at each split}\OperatorTok{,}\NormalTok{ consider only }\FunctionTok{sqrt}\NormalTok{(p) features}
\NormalTok{        trees}\OperatorTok{.}\FunctionTok{append}\NormalTok{(tree)}
    
    \ControlFlowTok{return}\NormalTok{ trees}

\KeywordTok{function} \FunctionTok{Predict}\NormalTok{(x}\OperatorTok{,}\NormalTok{ trees)}\OperatorTok{:}
\NormalTok{    predictions }\OperatorTok{=}\NormalTok{ [tree}\OperatorTok{.}\FunctionTok{predict}\NormalTok{(x) }\ControlFlowTok{for}\NormalTok{ tree }\KeywordTok{in}\NormalTok{ trees]}
    \ControlFlowTok{return} \FunctionTok{majority\_vote}\NormalTok{(predictions)              \# or }\FunctionTok{mean}\NormalTok{() }\ControlFlowTok{for}\NormalTok{ regression}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{for}\NormalTok{ each unvisited point p}\OperatorTok{:}
\NormalTok{    neighbors }\OperatorTok{=}\NormalTok{ all points within eps }\KeywordTok{of}\NormalTok{ p}
    
    \ControlFlowTok{if} \OperatorTok{|}\NormalTok{neighbors}\OperatorTok{|} \OperatorTok{\textless{}}\NormalTok{ min\_samples}\OperatorTok{:}
\NormalTok{        label p }\ImportTok{as}\NormalTok{ NOISE}
    \ControlFlowTok{else}\OperatorTok{:}
\NormalTok{        create }\KeywordTok{new}\NormalTok{ cluster C}
\NormalTok{        add p to C}
        \FunctionTok{expandCluster}\NormalTok{(p}\OperatorTok{,}\NormalTok{ neighbors}\OperatorTok{,}\NormalTok{ C}\OperatorTok{,}\NormalTok{ eps}\OperatorTok{,}\NormalTok{ min\_samples)}

\KeywordTok{function} \FunctionTok{expandCluster}\NormalTok{(p}\OperatorTok{,}\NormalTok{ neighbors}\OperatorTok{,}\NormalTok{ C}\OperatorTok{,}\NormalTok{ eps}\OperatorTok{,}\NormalTok{ min\_samples)}\OperatorTok{:}
    \ControlFlowTok{for}\NormalTok{ each q }\KeywordTok{in}\NormalTok{ neighbors}\OperatorTok{:}
        \ControlFlowTok{if}\NormalTok{ q is NOISE}\OperatorTok{:}\NormalTok{ add q to }\FunctionTok{C}\NormalTok{ (border point)}
        \ControlFlowTok{if}\NormalTok{ q is unvisited}\OperatorTok{:}
\NormalTok{            mark q }\ImportTok{as}\NormalTok{ visited}
\NormalTok{            q\_neighbors }\OperatorTok{=}\NormalTok{ all points within eps }\KeywordTok{of}\NormalTok{ q}
            \ControlFlowTok{if} \OperatorTok{|}\NormalTok{q\_neighbors}\OperatorTok{|} \OperatorTok{\textgreater{}=}\NormalTok{ min\_samples}\OperatorTok{:}\NormalTok{  \# q is also a core point}
\NormalTok{                neighbors }\OperatorTok{=}\NormalTok{ neighbors U q\_neighbors  \# expand search}
            \ControlFlowTok{if}\NormalTok{ q not }\KeywordTok{in}\NormalTok{ any cluster}\OperatorTok{:}
...
\visualfigure{../figures/aiml-02-classic/02-confusion-matrix.pdf}{Every classifier error is a false positive or false negative. Precision penalizes FPs, recall penalizes FNs — which matters more depends on the cost of each mistake.}
\end{document}
